...
export class QuestManager {
   ...
   */
  static async acceptQuest(userId: string, questId: string): Promise<void> {
    const { error } = await supabase
      .from('user_quests')
      .upsert({
        user_id: userId,
    ...
    if (error) throw new Error(`Failed to accept quest: ${error.message}`);
  }
   ...
   */
  static async abandonQuest(userId: string, questId: string): Promise<void> {
    const { error } = await supabase
      .from('user_quests')
      .delete()
      .match({ user_id: userId, quest_id: questId });

    if (error) throw new Error(`Failed to abandon quest: ${error.message}`);
  }
