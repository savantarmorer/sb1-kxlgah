...
-- Add RLS policies for the seeded data
DO $$ 
BEGIN
    -- Items policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'items' AND policyname = 'enable_read_access_for_all_users') THEN
        CREATE POLICY enable_read_access_for_all_users ON public.items FOR SELECT USING (true);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'items' AND policyname = 'enable_write_access_for_admin_users') THEN
        CREATE POLICY enable_write_access_for_admin_users ON public.items FOR ALL USING (
            EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND is_super_admin = true)
        );
    END IF;

    -- Daily rewards policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'daily_rewards' AND policyname = 'enable_read_access_for_all_users') THEN
        CREATE POLICY enable_read_access_for_all_users ON public.daily_rewards FOR SELECT USING (true);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'daily_rewards' AND policyname = 'enable_write_access_for_admin_users') THEN
        CREATE POLICY enable_write_access_for_admin_users ON public.daily_rewards FOR ALL USING (
            EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND is_super_admin = true)
        );
    END IF;
END $$; 