...
import { BattleStateEnum } from '@/types/battle.ts';
...
export const gameActions = {
  ...
  handleItemTransaction: async (
    ...
    try {
      ...
      const currentInventory = state.inventory?.items?.filter(i => i.quantity && i.quantity > 0) || [];
      ...
      const finalInventory = updatedInventory.filter(item => item.quantity && item.quantity > 0);
      ...
      if (type === 'use' && existingItemIndex >= 0 && updatedInventory[existingItemIndex]?.quantity && updatedInventory[existingItemIndex]?.quantity <= 0) {
  ...
  handleBattleAction: async (
    ...
    try {
      ...
      if (state.battle && state.battle.status === BattleStateEnum.COMPLETED) {
        const { error } = await supabase
          .from('battle_stats')
          .upsert({
            user_id: state.user.id,
