...
export function BattleItemDisplay({ items, onUseItem, disabled }: BattleItemDisplayProps) {
  ...
  const { phase, setRewards } = useBattle();
  ...
  const battleItems = items.filter(item => {
    ...
    console.log('[BattleItemDisplay] Checking item effects:', {
      itemId: item.id,
      itemName: item.name,
      effectsType: typeof effects,
      isArray: Array.isArray(effects),
      effects
    });
    ...
    const hasBattleEffects = 
      ...
      (Array.isArray(effects) && effects.some(effect => {
        const isBattle = isBattleEffect((effect as ItemEffect).type) ||
          (effect as ItemEffect).metadata?.battle_only;
        console.log('[BattleItemDisplay] Array effect check:', {
          effectType: (effect as ItemEffect).type,
          isBattleEffect: isBattleEffect((effect as ItemEffect).type),
          hasBattleMetadata: (effect as ItemEffect).metadata?.battle_only,
          isBattle
        });
        return isBattle;
      })) ||
  ...
  const handleItemClick = (item: InventoryItem) => {
    ...
    // Example: Apply item effect based on current phase
    if (phase === BattleStateEnum.CARD_SELECTION) {
      // Apply elimination effect
      if (item.effects.includes('eliminate_wrong_answer')) {
        // Logic to eliminate wrong answers
      }
      // Update rewards if necessary
      const newRewards = calculateBattleRewards(/* arguments based on effect */);
