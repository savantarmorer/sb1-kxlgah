  ...
  for (const [key, value] of Object.entries(update)) {
    ...
    switch (key) {
      ...
      case 'coins':
        ...
        if (key === 'level' && value === 0) return false;
        break;
      case 'inventory':
      ...
      case 'score':
        ...
        if (typeof value.player !== 'number' || !Number.isInteger(value.player)) return false;
        if ('opponent' in value && (typeof value.opponent !== 'number' || !Number.isInteger(value.opponent))) return false;
        break;
    }
  }
