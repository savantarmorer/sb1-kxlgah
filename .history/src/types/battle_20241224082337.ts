 ...
 * Battle Action Types
 */
export type BattleAction = 'ataque' | 'defesa' | 'contra_ataque';

export const ACTION_ADVANTAGES: Record<BattleAction, BattleAction> = {
  'ataque': 'contra_ataque',
  'defesa': 'ataque',
  'contra_ataque': 'defesa'
};
...
export const INITIAL_HAND_CONFIG = {
  ataque: 2,
  defesa: 2,
  contra_ataque: 2,
  wildcard: 1
...
export const calculateDamage = (attacker: PlayerState, defender: PlayerState): number => {
  if (!attacker.isCorrect) return 0;
  const baseDamage = Math.ceil((attacker.timeLeft / 30) * 20); // 30 seconds is max time
  return baseDamage;
};
