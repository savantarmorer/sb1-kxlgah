 ...
 */
export type BattleStatus = BattlePhase;
 ...
 */
export const initialBattleState: Partial<BattleState> = {
  status: 'idle',
  current_question: 0,
  total_questions: 0,
  questions: [],
...
export interface BattleError extends Error {
  timestamp: number;
  code?: string;
  details?: any;
}

export interface BattleState extends Omit<BaseBattleState, 'error' | 'metadata'> {
  status: BattleStatus;
  current_question: number;
  total_questions: number;
  questions: Question[];
...
export interface ExtendedBattleState extends Omit<BattleState, 'opponent' | 'metadata'> {
  opponent: BattleOpponent | null;
  metadata: Required<BattleMetadata>;
}

export interface EnhancedBattleState extends Omit<ExtendedBattleState, 'opponent' | 'metadata'> {
  opponent: BattleOpponent | null;
  metadata: Required<BattleMetadata>;
}
...
export const BattleResultsComponent: React.FC<BattleResultsProps> = () => null;
