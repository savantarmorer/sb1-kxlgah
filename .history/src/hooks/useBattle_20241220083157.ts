        ...
        battleChannel
          ...
          .on('broadcast', { event: 'question_results' }, ({ payload }) => {
            ...
            setBattleState(prev => ({
              ...prev,
              lastAnswers: {
                player: playerStates[userId]?.answer,
                opponent: playerStates[battle.opponent_id]?.answer,
                correct: currentQuestion.correct_answer
              },
              scores: {
                player: playerStates[userId]?.score || 0,
          ...
          .on('broadcast', { event: 'battle_complete' }, ({ payload }) => {
            ...
            setBattleState(prev => ({
              ...
              scores: {
                player: payload.finalScores[userId]?.score || 0,
                opponent: payload.finalScores[battle.opponent_id]?.score || 0
              }
            }));
          });
