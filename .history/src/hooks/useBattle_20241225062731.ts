...
export function useBattle() {
  ...
  const calculate_xp_gained = useCallback((score: number, total_questions: number): number => {
    const difficulty_multiplier = state.battle?.metadata?.difficulty === 'hard' ? 1.2 : 
                                state.battle?.metadata?.difficulty === 'easy' ? 0.8 : 1;
  ...
  }, [state.battle?.metadata?.difficulty, state.user?.streak]);
  ...
  const answer_question = useCallback(async (selected_answer: string) => {
    ...
    try {
      ...
      dispatch({ type: 'SET_BATTLE_PHASE', payload: 'RESOLUTION' });
  ...
  return {
    ...
    playerState: state.battle?.player_state,
    opponentState: state.battle?.opponent_state,
    currentQuestion: state.battle?.current_question,
    timeLeft: state.battle?.time_left,
    isReady,
    initializeBattle: initialize_battle,
    handleAnswer: answer_question,
    handleResolutionComplete: handle_battle_completion,
    setPhase: (phase: BattlePhase) => dispatch({ type: 'SET_BATTLE_PHASE', payload: phase }),
    getBattleRewards: get_battle_rewards,
    calculateXpGained: calculate_xp_gained,
    calculateCoinsEarned: calculate_coins_earned
  };
}