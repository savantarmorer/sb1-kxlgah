...
export function useBattle() {
  ...
  useEffect(() => {
    const checkIfReady = () => {
      const conditions = {
        ...
        !isAuthLoading,
        !!user || !!authUser,
        !!state.battle_stats // Make sure battle stats are loaded
      };

      const isSystemReady = Object.values(conditions).every(Boolean);
      console.debug('[useBattle] Checking system ready:', conditions);
      
      setIsReady(isSystemReady);
    };

    checkIfReady();
  }, [authInitialized, isAuthLoading, user, authUser, state.battle_stats]);
  ...
  const initializeBattle = useCallback(async (options?: BattleOptions) => {
    try {
      ...
      if (!isReady) {
        console.error('[useBattle] Battle system not ready. Waiting for initialization...');
        // Instead of throwing immediately, wait a bit to see if the system becomes ready
        await new Promise((resolve) => setTimeout(resolve, 1000));
        
        if (!isReady) {
          throw new Error('Battle system not ready after waiting');
        }
      }
