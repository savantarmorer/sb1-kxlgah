  ...
  const handle_battle_completion = useCallback(async (results: BattleResults) => {
    ...
    try {
      ...
      dispatch({
        ...
        payload: {
          xp: (state.user?.xp || 0) + battle_rewards.xp_earned,
          coins: (state.user?.coins || 0) + battle_rewards.coins_earned,
          level: calculate_level_xp(state.user?.xp || 0),
          streak: results.victory ? (state.user?.streak || 0) + 1 : 0
        }
      });
      ...
      const message = results.victory ? 
        t('battle.victory_rewards', { 
          xp: battle_rewards.xp_earned, 
          coins: battle_rewards.coins_earned 
        }) :
        results.draw ?
