  ...
  const syncQuests = useCallback(async () => {
    ...
    try {
      ...
      const formattedQuests = questsData?.map(quest => ({
        ...
        rewards: [
          { type: 'xp', value: quest.xp_reward },
          { type: 'coins', value: quest.coin_reward }
        ]
      })) as Quest[];
      ...
      const activeQuests = formattedQuests.filter(q => q.status !== QuestStatus.COMPLETED);
      const completedQuests = formattedQuests.filter(q => q.status === QuestStatus.COMPLETED);
