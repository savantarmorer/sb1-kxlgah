...
export function useBattle() {
  ...
  useEffect(() => {
    const handleCardSelectionComplete = async () => {
      if (phase === BattleStateEnum.CARD_SELECTION && selectedCard && opponentSelectedCard) {
        ...
        if (!question) {
          console.error('[useBattle] No question available at index:', currentQuestionIndex, {
        ...
        if (success) {
          try {
            ...
            console.debug('[useBattle] Current question set successfully:', question);
          } catch (error) {
            console.error('[useBattle] Failed to set current question:', error);
            showError(t('battle.error.question_setup'));
            await setPhaseWithValidation(BattleStateEnum.ERROR);
          }
        } else {
  ...
  }, [phase, selectedCard, opponentSelectedCard, state.battle?.questions, state.battle?.current_question, setPhaseWithValidation, setCurrentQuestion, showError, t]);
