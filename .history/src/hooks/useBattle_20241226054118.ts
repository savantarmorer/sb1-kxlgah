...
export function useBattle() {
  ...
  }, [dispatch, play_sound, showBattleNotification, state.battle?.status, t, user, authUser, isReady, authInitialized, isAuthLoading, initializeDecks, distributeCards, setPhase]);
  ...
  const handleAnswer = useCallback((answer: string) => {
    ...
    // Remove used cards
    removeUsedCards();
  }, [currentQuestion, selectedCard, opponentSelectedCard, timeLeft, streak, play_sound, removeUsedCards, state.battle?.metadata?.difficulty, playerHand, opponentHand]);
  ...
  const handleResolutionComplete = useCallback(() => {
    ...
    const nextQuestionIndex = state.battle?.current_question + 1;
    const nextQuestion = state.battle?.questions?.[nextQuestionIndex];

    if (nextQuestion) {
      // Update the current question in the battle state
      dispatch({
        type: 'UPDATE_BATTLE',
        payload: {
          ...state.battle,
  ...
  }, [opponentState.health, playerState.health, distributeCards, state.battle, dispatch]);
