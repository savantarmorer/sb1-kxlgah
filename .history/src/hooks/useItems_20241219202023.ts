...
import { GameItem, InventoryItem } from '../types/items';
...
export function useItems() {
   ...
   * 
   * @returns Promise<InventoryItem[]> Array of items
   * 
   * Used by:
  ...
  const fetchItems = async (): Promise<InventoryItem[]> => {
    try {
      ...
      const items = data?.map(convertItemFromDB).map(item => ({
        ...item,
        equipped: false,
        quantity: 0
      })) || [];
   ...
   * @returns Promise<InventoryItem | undefined> Saved item
   * 
   * Used by:
   * - ItemEditor component
   * - ItemManager component
   */
  const saveItem = async (itemData: Partial<GameItem>): Promise<InventoryItem | undefined> => {
    try {
      setError(null);
      ...
      if (data) {
        const convertedItem = {
          ...convertItemFromDB(data),
          equipped: false,
          quantity: 0
        };
        dispatch({
          type: 'UPDATE_INVENTORY',
          payload: { items: [convertedItem] }
        });
