...
jest.mock('../../contexts/GameContext', () => {
  ...
  const MockGameProvider = ({ children }: { children: React.ReactNode }) => {
    const GameContext = jest.requireActual('../../contexts/GameContext').GameContext;
    return React.createElement(GameContext.Provider, { value: mockContext }, children);
  };
...
describe('useBattle Hook', () => {
  ...
  it('initializes with correct state', () => {
    const { result } = renderHook(() => useBattle(), {
      wrapper: ({ children }) => {
        const GameProvider = jest.requireActual('../../contexts/GameContext').GameProvider;
