...
export function useBattle() {
  ...
  const user = gameState.user || authUser;
  ...
  useEffect(() => {
    const checkSystemReady = () => {
      const systemConditions = {
        ...
        hasStats: Boolean(gameState.battle_stats),
        hasGameState: Boolean(gameState.user)
      };
  ...
  }, [authInitialized, isAuthLoading, user, authUser, gameState.battle_stats, gameState.user]);

  const get_battle_rewards = useCallback((): BattleRewards => {
    if (!gameState.battle?.rewards) {
      return {
        xp_earned: 0,
        coins_earned: 0,
        streak_bonus: 0,
    ...
    return gameState.battle.rewards;
  }, [gameState.battle?.rewards]);

  const calculate_xp_gained = useCallback((score: number, total_questions: number): number => {
    const difficulty = gameState.battle?.metadata?.difficulty || 'medium';
    const streak_multiplier = gameState.user?.streak || 0;
    
    const rewards = calculateBattleRewards(
      score > 0,  // isCorrect
      gameState.battle?.time_left || 0,  // timeLeft
      streak_multiplier,
      difficulty
    );
    
    return rewards.xp_earned;
  }, [gameState.battle?.metadata?.difficulty, gameState.battle?.time_left, gameState.user?.streak]);
  ...
  useEffect(() => {
    const handleCardSelectionComplete = async () => {
      if (phase === BattleStateEnum.CARD_SELECTION && selectedCard && opponentSelectedCard) {
        const currentQuestionIndex = gameState.battle?.current_question ?? 0;
        const question = gameState.battle?.questions?.[currentQuestionIndex];
        ...
        if (question) {
          ...
          if (success) {
            ...
            console.error('[useBattle] Failed to transition to CARD_REVEAL phase');
  ...
  }, [phase, selectedCard, opponentSelectedCard, gameState.battle?.questions, gameState.battle?.current_question, setPhaseWithValidation, showError, t]);
  ...
  const initializeBattle = useCallback(async (options?: BattleOptions) => {
    try {
      ...
      console.debug('[useBattle] Starting battle initialization...', { 
        ...
        user: user,
        battleStats: gameState.battle_stats,
  ...
  }, [dispatch, play_sound, showBattleNotification, gameState.battle?.status, t, user, authUser, isReady, authInitialized, isAuthLoading]);
  ...
  const handle_battle_completion = useCallback(async (results: BattleResults) => {
    ...
    try {
      ...
      if (gameState.battle?.status === BattleStateEnum.COMPLETED) {
      ...
      const battle_rewards = LevelSystem.calculate_complete_battle_rewards(
        ...
        gameState.battle_stats?.difficulty || 1,
        gameState.user?.streak || 0,
        gameState.battle?.time_left || 0
      );
      ...
      try {
        await check_achievements(
          'battle_complete',
          gameState.battle?.score?.player || 0
        );
  ...
  }, [user, gameState.battle, gameState.battle_stats, gameState.user?.streak, check_achievements, showBattleNotification, t, play_sound, dispatch]);
  ...
  const answer_question = useCallback(async (selected_answer: string) => {
    ...
    try {
      ...
      const current_battle_stats: battle_statsDB = {
        total_battles: gameState.battle_stats?.total_battles || 0,
        wins: gameState.battle_stats?.wins || 0,
        losses: gameState.battle_stats?.losses || 0,
        win_streak: gameState.battle_stats?.win_streak || 0,
        highest_streak: gameState.battle_stats?.highest_streak || 0,
        total_xp_earned: gameState.battle_stats?.total_xp_earned || 0,
        total_coins_earned: gameState.battle_stats?.total_coins_earned || 0,
        difficulty: gameState.battle_stats?.difficulty || 1
      };

      const current_question = gameState.battle?.questions[gameState.battle.current_question];
      if (!current_question) {
      ...
      const battle_state: GameState = {
        ...
        user: gameState.user,
        recentXPGains: gameState.recentXPGains,
        leaderboard: gameState.leaderboard,
        login_history: gameState.login_history,
        ...
        syncing: gameState.syncing,
        showLevelUpReward: gameState.showLevelUpReward,
        current_levelRewards: gameState.current_levelRewards,
      ...
      const isLastQuestion = gameState.battle?.current_question === (gameState.battle?.questions?.length || 0) - 1;
      if (isLastQuestion) {
  ...
  }, [gameState, dispatch, handle_battle_completion, showError, t]);
  ...
  const handleAnswer = useCallback(async (answer: string) => {
    ...
    try {
      ...
      const rewards = calculateBattleRewards(
        ...
        timeLeft,
        streak,
        (gameState.battle?.metadata?.difficulty as 'easy' | 'medium' | 'hard') || 'medium'
      );
  ...
  }, [currentQuestion, selectedCard, opponentSelectedCard, timeLeft, streak, play_sound, gameState.battle?.metadata?.difficulty, playerHand, opponentHand, answer_question, setPhaseWithValidation, showError, t, playerState.isReady]);
  ...
  const handleResolutionComplete = useCallback(async () => {
    ...
    try {
      ...
      const nextQuestion = gameState.battle?.questions?.[nextQuestionIndex];

      if (nextQuestion && gameState.battle) {
  ...
  }, [opponentState.health, playerState.health, distributeCards, gameState.battle, dispatch, setSelectedAnswer, setIsAnswerCorrect, setTimeLeft, setPhaseWithValidation, showError, t]);
