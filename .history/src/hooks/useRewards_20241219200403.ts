...
import { Achievement, AchievementTrigger, AchievementTriggerType } from '../types/achievements';
...
export function useRewards() {
  ...
  const claimReward = (reward: Reward) => {
    ...
    dispatch({
      type: claimPayload.type === 'coins' ? 'ADD_COINS' : 'ADD_XP',
      payload: {
        amount: claimPayload.value,
        source: 'reward'
      }
    });
    ...
    if (reward.rarity === 'legendary') {
      ...
      const achievement: Achievement = {
        ...achievementPayload,
        order_num: achievementPayload.order,
        progress: 100,
        icon: '🏆',
