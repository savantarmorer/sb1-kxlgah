...
export function useBattle() {
  ...
  const get_current_question = useCallback((): BattleQuestion | null => {
    ...
    const question = state.battle.questions[state.battle.current_question] as BattleQuestion;
    return question;
  }, [state.battle]);
  ...
  const handle_item_effect = useCallback((effect: ItemEffect) => {
    ...
    switch (effect.type) {
      case 'eliminate_wrong_answer': {
        const currentQuestion = state.battle.questions[state.battle.current_question] as BattleQuestion;
