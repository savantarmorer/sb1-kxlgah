...
export function useInventory() {
    ...
    dispatch({
      ...
      payload: {
        user_inventory: type === 'purchase'
          ...
          : (state.user.user_inventory || []).map(i => 
              i.id === item.id 
  ...
  const equipItem = useCallback(async (item: GameItem) => {
    dispatch({
      type: 'UPDATE_USER_PROFILE',
      payload: {
        user_inventory: state.user.user_inventory.map(invItem =>
          invItem.id === item.id
  ...
  }, [dispatch, state.user?.user_inventory]);

  const unequipItem = useCallback(async (item: GameItem) => {
    dispatch({
      type: 'UPDATE_USER_PROFILE',
      payload: {
        user_inventory: state.user.user_inventory.map(i =>
          i.id === item.id
            ? { ...i, equipped: false }
            : i
        )
