...
export function useBattle() {
  ...
  const handleAnswer = useCallback(async (answer: string, selectedCard: string, opponentSelectedCard: string, playerHand: Card[], opponentHand: Card[]) => {
    ...
    await Promise.all([
      ...
      new Promise<void>(resolve => {
        ...
        if (isCorrect) {
          ...
          setScore(prev => ({
            ...prev,
            player: prev.player + calculateBattleRewards(isCorrect, timeLeft, streak, currentQuestion.difficulty).total_xp
          }));
        } else {
          setStreak(0);
        }
    ...
    // Move to resolution phase last, after all state updates
