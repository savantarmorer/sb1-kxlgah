...
export function useBattle() {
  ...
  const validatePhaseTransition = useCallback((from: BattlePhase, to: BattlePhase) => {
    const validTransitions: Record<BattlePhase, BattlePhase[]> = {
      ...
      [BattlePhase.DEALING]: [BattlePhase.CARD_SELECTION],
      [BattlePhase.CARD_REVEAL]: [BattlePhase.RESOLUTION]
    };
  ...
  useEffect(() => {
    if (state.battle?.status === 'active' && phase === BattlePhase.INITIALIZING) {
  ...
  useEffect(() => {
    const handleCardSelectionComplete = async () => {
      if (phase === BattlePhase.CARD_SELECTION && selectedCard && opponentSelectedCard) {
        ...
        if (question) {
          console.debug('[useBattle] Both cards selected, transitioning to QUESTION phase', {
  ...
  const handleAnswer = useCallback(async (answer: string) => {
    ...
    try {
      ...
      await setPhaseWithValidation(BattlePhase.RESOLUTION);
  ...
  const handleResolutionComplete = useCallback(async () => {
    ...
    try {
      ...
      if (opponentState.health <= 0 || playerState.health <= 0) {
        await setPhaseWithValidation(BattlePhase.COMPLETED);
        return;
      }
      ...
      if (nextQuestion && state.battle) {
        // Update battle state with next question and wait for completion
        await new Promise<void>(resolve => {
          dispatch({
        ...
        // Reset states for next round in parallel
        await Promise.all([
          new Promise<void>(resolve => {
            setSelectedAnswer('');
            resolve();
        ...
        // Distribute new cards and wait for completion
        await distributeCards();
        
        // Finally transition to card selection
        await setPhaseWithValidation(BattlePhase.CARD_SELECTION);
      } else {
