...
const CARD_DESCRIPTIONS: Record<BattleAction | 'wildcard', string[]> = {
  ataque: [
  ...
  wildcard: [
    'Carta coringa que pode ser usada como qualquer ação'
  ]
};
...
export const createDeck = (): BattleCard[] => {
  ...
  // Add wildcard
  if (INITIAL_HAND_CONFIG.wildcard > 0) {
    ...
    deck.push({
      ...
      isWildcard: true,
      isUsed: false,
      power: Math.floor(Math.random() * 25) + 15, // Slightly higher power range for wildcard (15-40)
      description: CARD_DESCRIPTIONS.wildcard[0]
    });
  }
 ...
 * Now allows for duplicate cards in hands
 */
export const dealHand = (deck: BattleCard[]): BattleCard[] => {
  const shuffledDeck = shuffleDeck([...deck]);
  return shuffledDeck.slice(0, INITIAL_HAND_CONFIG.hand_size).map(card => ({
    ...card,
    id: uuidv4() // Give each card a unique ID even if it's a duplicate
  }));
};
