...
const BASE_DECK: Card[] = [
  // Attack cards
  { id: 'atk1', action: BattleAction.ATTACK, power: 3 },
  { id: 'atk2', action: BattleAction.ATTACK, power: 4 },
  { id: 'atk3', action: BattleAction.ATTACK, power: 5 },
  
  // Defense cards
  { id: 'def1', action: BattleAction.DEFENSE, power: 3 },
  { id: 'def2', action: BattleAction.DEFENSE, power: 4 },
  
  // Counter cards
  { id: 'cnt1', action: BattleAction.COUNTER, power: 3 },
  { id: 'cnt2', action: BattleAction.COUNTER, power: 4 }
];
...
export function getAICardSelection(
  ...
  const attackCards = hand.filter(card => card.action === BattleAction.ATTACK);
  const defenseCards = hand.filter(card => card.action === BattleAction.DEFENSE);
  const counterCards = hand.filter(card => card.action === BattleAction.COUNTER);

  // Low health strategy (below 40% health)
  if (currentHealth <= 20) {
    // Prefer counter if opponent is attacking
    if (opponentCard.action === BattleAction.ATTACK && counterCards.length > 0) {
  ...
  if (currentHealth > 20) {
    ...
    if (opponentCard.action === BattleAction.ATTACK && defenseCards.length > 0) {
  ...
  // Default strategies
  if (opponentCard.action === BattleAction.DEFENSE && attackCards.length > 0) {
