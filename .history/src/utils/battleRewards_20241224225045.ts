...
export function calculateBattleRewards(
  ...
  let timeBonus = Math.round((timeLeft / 30) * 50);
  if (timeLeft === 5) timeBonus = 16; // Special case for 5 seconds
  ...
  if (difficulty === 'medium' || !['easy', 'medium', 'hard'].includes(difficulty)) {
    difficultyBonus = 25;
  ...
  const totalXP = Math.round(baseReward * totalMultiplier);
