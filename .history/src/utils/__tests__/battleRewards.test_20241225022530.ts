...
describe('Battle Rewards Calculation', () => {
  it('calculates base rewards for correct answer', () => {
    ...
    expect(rewards.total_xp).toBe(275); // Base 175 + 57% bonus (50% time + 0% streak + 7% medium difficulty)
    expect(rewards.total_coins).toBe(137); // Half of XP rounded down
  });
  ...
  it('handles edge cases', () => {
    ...
    expect(invalidDifficultyRewards.total_xp).toBe(mediumRewards.total_xp);
  });
}); 