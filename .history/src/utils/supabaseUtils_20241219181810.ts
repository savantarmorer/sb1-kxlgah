...
export function convertQuestFromDB(dbQuest: any): Quest {
  return {
    ...
    rewards: [
      { type: 'xp', value: dbQuest.xp_reward, metadata: {} },
      { type: 'coins', value: dbQuest.coin_reward, metadata: {} }
    ],
    created_at: dbQuest.created_at || new Date().toISOString(),
    updated_at: dbQuest.updated_at || new Date().toISOString()
  };
}
 ...
 */
export function convertItemToDB(item: Partial<GameItem & { requirements?: ItemRequirement[] }>): Record<string, any> {
