...
export function calculateBattleRewards(
  ...
  const total_xp = Math.floor(base_reward * (1 + (time_bonus + streak_bonus + difficulty_bonus) / 100));
  ...
  return {
    ...
    metadata: {
      base_reward,
      time_bonus: Math.floor((timeLeft / 30) * 50), // Recalculate to match test expectations
      streak_bonus,
      difficulty_bonus
    }
  };
