...
import type { BattleResults, BattleRewards, BattleScore, BattleState, EnhancedBattleState, BattleQuestion } from '../types/battle';
...
export const calculateBattleResults = (
  ...
  const newScore = calculateBattleScore(
    { player: battle.score_player || 0, opponent: battle.score_opponent || 0 },
    isCorrect
  );
  ...
  console.debug('[Battle Debug] Battle Results:', {
    ...
    rewards,
    questionProgress: {
      current: (battle.current_question || 0) + 1,
      total: battle.total_questions || 0
    }
  });
  ...
  return {
    ...
    rewards,
    stats: {
      time_taken: BATTLE_CONFIG.time_per_question - (battle.time_left || 0),
      total_questions: battle.total_questions || 0,
      average_time: (BATTLE_CONFIG.time_per_question - (battle.time_left || 0)) / ((battle.current_question || 0) + 1),
