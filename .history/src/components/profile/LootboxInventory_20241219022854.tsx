...
const rarityGradients: Record<RewardRarity, string> = {
...
const rarityGlow: Record<RewardRarity, string> = {
...
export default function LootboxInventory() {
  ...
  // Get lootboxes from game state's pending_lootboxes and ensure they match LootboxItem type
  const lootboxes = (state.user?.pending_lootboxes || []).map(lootbox => ({
    ...lootbox,
    rarity: lootbox.rarity as RewardRarity // Type assertion since we know these values match
  }));
      ...
      for (const reward of rewards) {
        if (reward.type === 'xp' || reward.type === 'coins') {
          const { error } = await supabase
            .from('profiles')
            .update({
              [reward.type]: supabase.rpc('increment_counter', {
                column_name: reward.type,
                increment_amount: Number(reward.value)
              })
            })
            .eq('id', state.user?.id);
