...
export default function DailyRewardSystem(): JSX.Element | null {
   ...
   * - Reward system
   */
  const checkDailyLogin = useCallback((): void => {
    ...
    if (!lastLoginDate || !isToday(parseISO(lastLoginDate))) {
      ...
      if (!state.user.login_history?.includes(today)) {
        ...
        dispatch({ 
          ...
          payload: { 
            quest_progress: {
              login_history: [...(state.user.login_history || []), today]
            }
          } 
        });
  ...
  }, [state.user, state.user.login_history, lastLoginDate, DAILY_REWARDS, setLastLoginDate, dispatch, updateStreakMultiplier]);
