...
export function BattleMode({ on_close }: BattleModeProps) {
  ...
  const handleBattleCompletion = useCallback(async () => {
    ...
    try {
      ...
      // Create enhanced battle state with guaranteed rewards
      ...
      const enhancedBattleState = {
        ...
        metadata: {
          ...
          completed_at: battle_state.metadata?.completed_at || new Date().toISOString(),
          winner: battle_state.metadata?.winner || null,
          phase: BattlePhase.Completed,
      ...
      } as unknown as EnhancedBattleState;
