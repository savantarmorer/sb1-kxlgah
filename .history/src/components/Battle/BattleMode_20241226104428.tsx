...
export default function BattleMode({ mode = 'all', difficulty = 'medium', is_bot = true }: BattleModeProps) {
  ...
  useEffect(() => {
    ...
    if (phase === BattleStateEnum.CARD_SELECTION && selectedCard) {
      ...
      if (playerCard) {
        ...
        // Wait for opponent card selection to complete
        const timer = setTimeout(async () => {
          try {
            console.debug('[BattleMode] Opponent card selected, transitioning to card reveal');
            ...
            if (success) {
              ...
              if (question) {
                // Set the current question first
                await setCurrentQuestion(question);
                
                // Then transition to question phase after a delay
                const questionTransitionTimer = setTimeout(async () => {
                  try {
                    console.debug('[BattleMode] Card reveal complete, transitioning to question phase');
                    // We must be in CARD_REVEAL phase to transition to QUESTION
                    if (phase === BattleStateEnum.CARD_REVEAL) {
                      const transitionSuccess = await setPhaseWithValidation(BattleStateEnum.QUESTION);
                      if (!transitionSuccess) {
                        console.error('[BattleMode] Failed to transition to question phase: Invalid transition');
                        showError(t('battle.error.phase_transition'));
                      }
                    } else {
                      console.error('[BattleMode] Cannot transition to question phase - not in card reveal phase');
                      showError(t('battle.error.phase_transition'));
                    }
                  } catch (error) {
                    console.error('[BattleMode] Failed to transition to question phase:', error);
                    showError(t('battle.error.phase_transition'));
                  }
                }, 1500); // 1.5 seconds to show the cards
                
                // Update the questionTimer ref to allow cleanup
                questionTimer = questionTransitionTimer;
              } else {
                console.error('[BattleMode] No question available for index:', currentQuestionIndex);
                showError(t('battle.error.no_question'));
                await setPhaseWithValidation(BattleStateEnum.ERROR);
              }
            } else {
              console.error('[BattleMode] Failed to transition to card reveal: Invalid transition');
              showError(t('battle.error.phase_transition'));
            }
          } catch (error) {
            console.error('[BattleMode] Failed to transition to card reveal:', error);
            showError(t('battle.error.phase_transition'));
          }
        }, 500);
  ...
  const handleBattleStart = useCallback(async () => {
    try {
      ...
      // First reset to IDLE state
      dispatch({ type: 'SET_BATTLE_STATUS', payload: BattleStateEnum.IDLE });
      await setPhaseWithValidation(BattleStateEnum.IDLE);
      
      // Then transition to initializing
      dispatch({ type: 'SET_BATTLE_STATUS', payload: BattleStateEnum.INITIALIZING });
      await setPhaseWithValidation(BattleStateEnum.INITIALIZING);
      
      console.debug('[BattleMode] Initializing battle with options:', {
        difficulty,
        is_bot,
        category: mode === 'all' ? undefined : mode
      });
      ...
      // Initialize decks before transitioning to active state
      await initializeDecks();

      // Move to preparing phase
      await setPhaseWithValidation(BattleStateEnum.PREPARING);

      console.debug('[BattleMode] Decks initialized, transitioning to active state...');

      // Only transition to active state after initialization is complete
      dispatch({ type: 'SET_BATTLE_STATUS', payload: BattleStateEnum.ACTIVE });
      
      // Move to dealing phase
      await setPhaseWithValidation(BattleStateEnum.DEALING);

      console.debug('[BattleMode] Battle initialization complete.');
    } catch (error) {
      console.error('[BattleMode] Battle initialization failed:', error);
      dispatch({ type: 'SET_BATTLE_STATUS', payload: BattleStateEnum.ERROR });
      showError(t('battle.error.initialization'));
      await setPhaseWithValidation(BattleStateEnum.ERROR);
      setError('Failed to initialize battle. Please try again.');
    }
  }, [dispatch, difficulty, is_bot, mode, initializeBattle, initializeDecks, setPhaseWithValidation, showError, t]);
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Box sx={{ 
        ...
        <AnimatePresence mode="wait">
          ...
          {(phase === BattleStateEnum.CARD_SELECTION || phase === BattleStateEnum.CARD_REVEAL) && (
            <Box sx={{ 
              display: 'grid',
