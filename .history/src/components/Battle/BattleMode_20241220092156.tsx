...
const castBattleState = (state: BattleState | null): ExtendedBattleState | null => {
  ...
  const defaultState: ExtendedBattleState = {
    questions: [],
    current_question: 0,
    player_states: {},
    started_at: new Date().toISOString(),
    phase: BattlePhases.waiting
  };
  
  return {
    ...defaultState,
    ...state,
    phase: (state.metadata?.phase as BattlePhaseType) || BattlePhases.waiting,
    metadata: {
      ...state.metadata,
      phase: state.metadata?.phase || BattlePhases.waiting
    }
  };
};
