  ...
  const battleItems = items.filter(item => {
    ...
    const hasBattleEffects = 
      ...
      (!Array.isArray(effects) && effects && typeof effects === 'object' && (
        'type' in effects && isBattleEffect(effects.type) ||
        'boostType' in effects && effects.boostType === 'battle' ||
        'metadata' in effects && effects.metadata?.battle_only ||
        'battle_only' in effects && effects.battle_only
      ));
