...
export default function BattleMode({ mode = 'all' }: BattleModeProps) {
    ...
    if (isCorrect) {
      ...
      if (playerCard.action === 'attack') {
        setOpponentState(prev => ({
          ...prev,
          health: Math.max(0, prev.health - timeLeft)
        }));
        setCurrentDamage(timeLeft);
      } else if (playerCard.action === 'defense') {
        setPlayerState(prev => ({
          ...prev,
          shield: Math.min(50, prev.shield + timeLeft)
        }));
        setCurrentDamage(0);
  ...
  const initializeBattle = useCallback(async () => {
    try {
      ...
      console.log('Fetching questions for mode:', selectedMode);
      ...
      try {
        ...
        // Filter questions based on mode if needed
        ...
        if (selectedMode !== 'all') {
          ...
          if (categoryToFilter) {
            filteredQuestions = questions.filter(q => q.category === categoryToFilter);
            console.log(`Questions for ${selectedMode} mode (${categoryToFilter}):`, filteredQuestions.length);
          }
        }
        ...
        const transformedQuestions: BattleQuestion[] = filteredQuestions.map(q => {
          // Map the letter answer to the actual alternative text
          ...
          return {
            ...
            correct_answer: answerMap[q.correct_answer] || q.correct_answer,
            category: q.category,
        ...
        console.log('Selected questions for battle:', shuffledQuestions.length);
        console.log('First question sample:', shuffledQuestions[0]); // Debug log to verify transformation

        setQuestions(shuffledQuestions);
        setCurrentQuestion(shuffledQuestions[0]);
        setCurrentQuestionIndex(0);
        setTimeLeft(30); // Set time to 30 seconds
        ...
        showError(`Failed to fetch questions: ${dbError.message}`);
