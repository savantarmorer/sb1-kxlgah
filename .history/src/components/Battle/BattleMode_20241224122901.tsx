...
export default function BattleMode({ mode = 'all' }: BattleModeProps) {
  ...
  const handleAnswer = async (answer: string) => {
    ...
    if (isCorrect) {
      // Player attacks - damage based on time remaining and card power
      const timeMultiplier = timeLeft / BATTLE_CONFIG.time_per_question; // 0 to 1
      const baseDamage = Math.round(playerCard.power * (0.5 + timeMultiplier)); // 50% to 150% of card power
      const result = calculateDamage(
        { ...playerCard, power: baseDamage },
        opponentCard
      );
      calculatedDamage = result.damage;
      setOpponentState(prev => ({
        ...prev,
        health: Math.max(0, prev.health - calculatedDamage)
      }));
    } else {
      // Opponent attacks with normal damage
      const result = calculateDamage(opponentCard, playerCard);
      calculatedDamage = result.damage;
      setPlayerState(prev => ({
        ...prev,
        health: Math.max(0, prev.health - calculatedDamage)
    ...
    setCurrentDamage(calculatedDamage);
  };
  ...
  const initializeBattle = useCallback(async () => {
    try {
      ...
      // Fetch questions with proper ordering and filtering
      let query = supabase
        ...
        .select(`
          ...
          category
        `)
        .limit(50); // Fetch more questions than needed for randomization

      if (selectedMode !== 'all') {
        query = query.eq('category', selectedMode);
      }

      // Add random ordering to avoid getting the same questions
      ...
      if (!questions || questions.length === 0) {
        console.error('No questions found for mode:', selectedMode);
      ...
      const validQuestions = questions.filter(q => 
        ...
        q.alternative_d && 
        q.correct_answer &&
        ['alternative_a', 'alternative_b', 'alternative_c', 'alternative_d'].includes(q.correct_answer)
      );

      console.log('Valid questions:', validQuestions.length);

      if (validQuestions.length === 0) {
        throw new Error('No valid questions found. Please check question format.');
      }

      // Shuffle and select required number of questions
      const shuffledQuestions = [...validQuestions]
        .sort(() => Math.random() - 0.5)
        .slice(0, BATTLE_CONFIG.questions_per_battle)
        .map(q => ({
          ...q,
          answers: {
      ...
      console.log('Selected questions:', shuffledQuestions);
