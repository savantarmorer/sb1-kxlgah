...
export default function BattleMode() {
  ...
  const { useItem, inventory } = useInventory();
  ...
  const potionIconUrl = '/images/items/potion-purple.png';
    ...
    try {
      ...
      const { data: questions, error } = await supabase
        ...
        .then(({ data, error }) => {
          ...
          // If we have data, randomly select questions from it
  ...
  const handleAnswer = async (answer: string) => {
    ...
    const opponentAccuracy = BATTLE_CONFIG.bot.base_accuracy + 
      (BATTLE_CONFIG.bot.accuracy_multiplier * (
        state.battle_stats?.difficulty || 
        BATTLE_CONFIG.matchmaking.default_rating
      ) / 1000);
  ...
  const handleUsePotion = async () => {
    try {
      await useItem('alternative_remover_potion_id');
      handlePotionEffect();
    } catch (error) {
      showError('Failed to use potion.');
    }
  };

  const handlePotionEffect = () => {
    if (currentQuestion) {
      const incorrectOptions = ['B', 'C', 'D'].filter(opt => opt !== currentQuestion.correct_answer);
      const optionsToEliminate = incorrectOptions.slice(0, 2);
      setCurrentQuestion(prev => {
        if (!prev) return null;
        return {
          ...prev,
          eliminated_options: optionsToEliminate,
          id: prev.id,
          question: prev.question,
          alternative_a: prev.alternative_a,
          alternative_b: prev.alternative_b,
          alternative_c: prev.alternative_c,
          alternative_d: prev.alternative_d,
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Box sx={{ 
        ...
        <Box sx={{ 
          ...
          <Box sx={{ 
            ...
            <Typography variant="body2" color="text.secondary">
              Rating: <span className="font-bold">{state.battle_stats?.difficulty || BATTLE_CONFIG.matchmaking.default_rating}</span>
            </Typography>
          </Box>
        </Box>
      ...
      {/* Alternative Remover Potion Icon */}
      <IconButton
        onClick={handleUsePotion}
        disabled={!selectedAnswer || !inventory.some(item => item.id === 'alternative_remover_potion_id')}
        sx={{ position: 'absolute', top: 16, right: 16 }}
        aria-label="Use Alternative Remover Potion"
      >
        <Avatar src={potionIconUrl} alt="Alternative Remover Potion" />
      </IconButton>
    </Container>
  );
}