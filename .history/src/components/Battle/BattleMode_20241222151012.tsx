...
export default function BattleMode() {
  ...
  const startMatchmaking = async (options: BattleOptions) => {
    ...
    try {
      setIsSearching(true);
      const matchmaking = new LiveMatchmakingService();
      
      // Subscribe to matchmaking updates
      matchmaking.subscribeToMatchUpdates((matchState: MatchState) => {
        if (matchState.status === BattleStatus.MATCHED && matchState.opponent) {
          setOpponent(matchState.opponent);
          setMatchId(matchState.matchId || null);
          void initializeBattle(matchState.matchId || null);
        }
      });

      // Join the queue
      await matchmaking.joinMatchmakingQueue(state.user.id, {
        id: state.user.id,
  ...
  const startBotBattle = useCallback(async (options: BattleOptions) => {
    if (!state.user) return;
    try {
  ...
  const handleUseAlternativeRemover = () => {
    ...
    const alternatives = ['alternative_a', 'alternative_b', 'alternative_c', 'alternative_d']
      .map(key => currentQuestion[key as keyof Pick<BattleQuestion, 'alternative_a' | 'alternative_b' | 'alternative_c' | 'alternative_d'>])
  ...
  if (showLobby) {
    const defaultStats: BattleStats = {
      ...
      tournament_rating: BATTLE_CONFIG.matchmaking.default_rating,
    ...
    return (
      <BattleLobby
        ...
        on_close={() => navigate(-1)}
        stats={state.battle_stats || defaultStats}
      />
    );
  }
