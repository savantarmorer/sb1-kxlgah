...
export function BattleMode({ on_close }: BattleModeProps) {
    ...
    try {
      ...
      const enhancedBattleState: EnhancedBattleState = {
        ...
        metadata: {
          ...
          difficulty: battle_state.metadata.difficulty,
          mode: 'practice',
          player_states: {},
          eliminated_options: battle_state.metadata.eliminated_options || [],
          started_at: new Date().toISOString(),
          completed_at: new Date().toISOString(),
          winner: battle_state.score.player > battle_state.score.opponent ? 'player' : 'opponent',
          total_questions: battle_progress.total_questions,
  ...
  const handle_use_item = useCallback(async (item: Item) => {
    try {
  ...
  // Convert inventory items to battle items
  const battleItems = inventory?.map(item => ({
    id: item.id,
    name: item.name,
