...
import { Box, Typography, Card, CardContent, Button, Modal, ModalOverlay, ModalContent, ModalHeader, ModalBody, VStack, HStack, Text, Progress } from '@mui/material';
...
export function BattleMode({ on_close }: BattleModeProps) {
  ...
  const renderPreBattleModal = () => {
    if (!state.battle?.opponent || !state.user) return null;

    const bothReady = preBattleState.playerReady && preBattleState.opponentReady;

    return (
      <Modal
        isOpen={showLobby && state.battle.status === 'matched'}
        onClose={() => {}}
              ...
              <HStack justify="space-between" w="100%" px={4}>
                <VStack>
                  ...
                  {preBattleState.playerReady && (
                    <Text color="green.500">{t('battle.pre_battle.player_ready')}</Text>
                  )}
                </VStack>
                <Text fontSize="2xl" fontWeight="bold">VS</Text>
                <VStack>
                  <PlayerCard player={state.battle.opponent} />
                  {preBattleState.opponentReady && (
                    <Text color="green.500">{t('battle.pre_battle.opponent_ready')}</Text>
                  )}
                </VStack>
              </HStack>
              ...
              {bothReady ? (
                <VStack w="100%" spacing={2}>
                  <Progress 
                    ...
                    colorScheme="blue"
                  />
                  <Text>{t('battle.pre_battle.prepare')}</Text>
                </VStack>
              ) : (
                <Button
                  colorScheme="green"
                  onClick={handleBattleConfirm}
                  isDisabled={preBattleState.playerReady}
                >
                  {preBattleState.playerReady 
