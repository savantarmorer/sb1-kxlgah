...
import { Box, Typography, Button, Modal, Stack, LinearProgress, Card, CardContent } from '@mui/material';
...
// Update BattleState interface
interface BaseBattleState {
...
type BattlePhaseType = typeof BattlePhases[keyof typeof BattlePhases];

interface ExtendedBattleState extends BaseBattleState {
...
const castBattleState = (state: BaseBattleState | null): ExtendedBattleState | null => {
  if (!state) return null;
  
  const { error, metadata, phase, ...rest } = state;
  const currentPhase = phase || metadata?.phase || BattlePhases.waiting;
  
  return {
    ...rest,
    phase: currentPhase as BattlePhaseType,
...
export function BattleMode({ on_close }: BattleModeProps) {
  ...
  const battle_state = castBattleState(rawBattleState as BaseBattleState);
  ...
  useEffect(() => {
    if (battle_error) {
      showError(handleError(battle_error));
    }
  }, [battle_error, showError]);

  // Update player answers handling
  const getPlayerAnswers = (state: ExtendedBattleState): boolean[] => {
                    ...
                    results={{
                      ...
                      stats: {
                        ...
                        average_time: battle_progress.total_questions > 0 
                          ...
                          : 0,
                        correct_answers: getPlayerAnswers(battle_state).filter(a => a).length
                      }
                    }}
