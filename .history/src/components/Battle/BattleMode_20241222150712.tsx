...
export default function BattleMode() {
  ...
  const startMatchmaking = async (options: any) => {
    try {
      setIsSearching(true);
      const service = new LiveMatchmakingService();
      
      // Subscribe to matchmaking updates
      service.onMatchStateChange((matchState) => {
        if (matchState.status === 'matched') {
          setOpponent(matchState.opponent);
          setMatchId(matchState.matchId);
          initializeBattle(matchState.matchId);
        }
      });

      // Join the queue
      await service.joinQueue(state.user.id, {
        rating: state.battle_stats?.rating || BATTLE_CONFIG.matchmaking.default_rating,
