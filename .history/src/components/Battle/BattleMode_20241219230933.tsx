  ...
  const handle_battle_completion = useCallback(async () => {
    ...
    try {
      ...
      const battleResults: BattleResults = {
        ...
        stats: {
          time_taken: BATTLE_CONFIG.time_per_question - (battle_state.time_left || 0),
          total_questions: battle_progress.total_questions,
          correct_answers: battle_state.player_answers.filter(a => a).length,
          average_time: (BATTLE_CONFIG.time_per_question - (battle_state.time_left || 0)) / battle_progress.total_questions
        }
      };
