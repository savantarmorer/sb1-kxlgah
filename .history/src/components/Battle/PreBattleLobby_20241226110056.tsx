...
interface PreBattleLobbyProps {
  ...
  setPhaseWithValidation: (phase: BattleStateEnum) => Promise<boolean>;
...
export function PreBattleLobby({ 
  ...
  const startTimeout = useCallback(async () => {
    try {
      // First ensure we're in PREPARING phase
      if (state.battle?.phase !== BattleStateEnum.PREPARING) {
      ...
      // Finally call onBattleStart
      if (onBattleStart) {
        onBattleStart();
      }
    } catch (error) {
      console.error('[PreBattleLobby] Failed to start battle:', error);
      showError(t('battle.error.start'));
      await setPhaseWithValidation(BattleStateEnum.ERROR);
    }
  }, [onBattleStart, setPhaseWithValidation, showError, t, state.battle?.phase]);
