...
export function BattleMode({ on_close }: BattleModeProps) {
        ...
        for (const effect of item.effects) {
          switch (effect.type) {
            case 'eliminate_wrong_answer':
              ...
              if (currentQuestion) {
                // First mark the options as eliminated
                dispatch({
  ...
  // Get current question with type guard
  const getCurrentQuestion = (): BattleQuestion | undefined => {
    if (!battle_state || battle_state.current_question < 0 || battle_state.current_question >= battle_state.questions.length) {
      return undefined;
    }
    return battle_state.questions[battle_state.current_question];
  };
