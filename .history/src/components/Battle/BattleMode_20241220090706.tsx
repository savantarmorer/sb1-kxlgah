...
export function BattleMode({ on_close }: BattleModeProps) {
  ...
  const handleBattleConfirm = useCallback(async () => {
    if (!battle_state?.matchId || !state.user?.id) return;

    try {
      await LiveMatchmakingService.confirmPlayerReady(
        battle_state.matchId,
        state.user.id
      );
    } catch (error) {
  ...
  }, [battle_state?.matchId, state.user?.id, showError, t]);

  // Listen for battle start
  useEffect(() => {
    if (!battle_state?.matchId) return;

    const channel = supabase.channel(`battle:${battle_state.matchId}`);
  ...
  }, [battle_state?.matchId, dispatch]);

  // Render pre-battle modal
  const renderPreBattleModal = () => {
    if (!battle_state?.opponent) return null;

    return (
      <Modal
        isOpen={showLobby && battle_state.status === 'matched'}
        onClose={() => {}}
        closeOnOverlayClick={false}
      >
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>{t('battle.pre_battle.title')}</ModalHeader>
          <ModalBody>
            <VStack spacing={4}>
              <HStack justify="space-between" w="100%">
                <PlayerCard player={state.user} />
                <Text fontSize="2xl" fontWeight="bold">VS</Text>
                <PlayerCard player={battle_state.opponent} />
              </HStack>
              <Button
                colorScheme="green"
