...
export default function BattleMode({ mode = 'all', difficulty = 'medium', is_bot = true }: BattleModeProps) {
  ...
  useEffect(() => {
    ...
    // Handle transition from CARD_REVEAL to QUESTION
    if (phase === BattleStateEnum.CARD_REVEAL && currentQuestion) {
      questionTimer = setTimeout(async () => {
        try {
          console.debug('[BattleMode] Card reveal complete, transitioning to question phase');
          const success = await setPhaseWithValidation(BattleStateEnum.QUESTION);
          if (!success) {
            console.error('[BattleMode] Failed to transition to question phase');
            showError(t('battle.error.phase_transition'));
          }
        } catch (error) {
      ...
      }, 2000); // 2 seconds to show the cards
  ...
  }, [phase, selectedCard, playerHand, selectOpponentCard, setPhaseWithValidation, setCurrentQuestion, state.battle?.current_question, state.battle?.questions, showError, t, currentQuestion]);
  ...
  const handleDistributionComplete = useCallback(async () => {
    try {
      ...
      // Update both battle status and phase
      dispatch({ type: 'SET_BATTLE_STATUS', payload: BattleStateEnum.CARD_SELECTION });
      
      // Then transition to card selection using the validated phase transition
  ...
  }, [distributeCards, setPhaseWithValidation, showError, t, dispatch]);
