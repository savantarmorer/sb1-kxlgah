...
const createEnhancedBattleState = (
  battle_state: ExtendedBattleState,
  ...
  const metadata: Required<BattleMetadata> = {
    ...
    phase: battle_state.metadata.phase || BattlePhase.Completed,
    opponent_id: battle_state.metadata.opponent_id || '',
    time_taken: battle_state.metadata.time_taken || 0,
    total_questions: battle_state.metadata.total_questions || battle_progress.total_questions,
    time_per_question: battle_state.metadata.time_per_question || BATTLE_CONFIG.time_per_question,
    in_progress: battle_state.metadata.in_progress || false
  };
  ...
  return {
    ...
    rewards: mapBattleRewards(rewards)
  };
};
...
export function BattleMode({ on_close }: BattleModeProps) {
  ...
  const handleBattleCompletion = useCallback(async () => {
    ...
    try {
      ...
      const enhancedBattleState = createEnhancedBattleState(
        battle_state,
        {
          total_questions: battle_progress.total_questions,
