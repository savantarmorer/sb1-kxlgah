...
export function QuestionDisplay({ question, on_answer, disabled, eliminated_options = [] }: QuestionDisplayProps) {
  ...
  ].filter(option => option.text && !eliminated_options.includes(option.id));
      ...
      <div className="grid grid-cols-1 gap-3">
        {options.map((option, index) => (
          <motion.button
            ...
            disabled={disabled || selectedAnswer !== null || eliminated_options.includes(option.id)}
            className={`
              ...
              ${selectedAnswer === option.id 
                ...
                : eliminated_options.includes(option.id)
                  ? 'bg-red-500/20 border-red-500/50 text-gray-500 cursor-not-allowed opacity-50'
