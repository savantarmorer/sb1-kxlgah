...
export default function BattleMode({ mode = 'all' }: BattleModeProps) {
  ...
  const handleAnswer = async (answer: string) => {
    ...
    let damage = 0;
    let newPlayerHealth = playerState.health;
    let newOpponentHealth = opponentState.health;

    if (isCorrect) {
      // Player attacks
      const result = calculateDamage(playerCard, opponentCard);
      damage = result.damage;
      newOpponentHealth = Math.max(0, opponentState.health - damage);
      ...
      damage = result.damage;
      newPlayerHealth = Math.max(0, playerState.health - damage);
      setPlayerState(prev => ({
        ...prev,
    ...
    return (
      <>
        {phase === BATTLE_PHASES.RESOLUTION && (
          <BattleResolution
            isCorrect={isCorrect}
            ...
            newOpponentHealth={newOpponentHealth}
