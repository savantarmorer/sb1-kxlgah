...
export default function BattleMode({ mode = 'all' }: BattleModeProps) {
  ...
  const processBattleRound = async (currentPlayerState: PlayerState, currentOpponentState: PlayerState) => {
    ...
    if (sameAction && currentPlayerState.isCorrect && currentOpponentState.isCorrect) {
      // Both players chose the same action and answered correctly
      // The player with more time left (faster response) deals damage
      const timeDifference = Math.abs(currentPlayerState.timeLeft - currentOpponentState.timeLeft);
      const damage = Math.ceil(timeDifference * 0.5); // Scale damage based on time difference
      
      if (currentPlayerState.timeLeft > currentOpponentState.timeLeft) {
        // Player was faster
        console.log('Player was faster, dealing time-based damage:', damage);
        ...
        // Opponent was faster
        console.log('Opponent was faster, dealing time-based damage:', damage);
        ...
        // Exactly same time - no damage
        console.log('Same timing - no damage dealt');
        setBattleResult({
          attacker: 'player',
          damage: 0
        });
      }
    } else if (playerAdvantage && currentPlayerState.isCorrect) {
      const damage = calculateDamage(currentPlayerState, currentOpponentState);
      const shieldBlock = Math.min(currentOpponentState.shield, damage);
      const actualDamage = Math.max(0, damage - shieldBlock);
      
      console.log('Player deals advantage damage:', { damage, shieldBlock, actualDamage });
      ...
      console.log('Opponent deals advantage damage:', { damage, shieldBlock, actualDamage });
