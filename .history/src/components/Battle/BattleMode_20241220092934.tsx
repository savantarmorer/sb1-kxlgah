...
interface BattleStateWithPhase extends BattleState {
  metadata?: {
    phase?: BattlePhase;
    [key: string]: any;
  };
      ...
      <Box sx={{ flex: 1, p: 3 }}>
        ...
        <Box sx={{ 
          ...
          <Box sx={{ display: 'flex', gap: 2 }}>
            <Button
              ...
              disabled={!rewards_claimed && isBattleCompleted(battle_state)}
        ...
        {showLobby ? (
          ...
          <Box sx={{ position: 'relative' }}>
            <AnimatePresence mode="wait">
              {battle_state && (
                <BattleStateTransition
                  status={battle_state.status}
                  message={isBattleError(battle_state) ? battle_state.error?.message : getBattleStatusMessage(battle_state.status)}
                  onComplete={() => {
                    if (isBattleCompleted(battle_state) && !rewards_claimed && !is_transitioning) {
            ...
            {/* Question display */}
            {current_question && isBattleActive(battle_state) && (
              <motion.div
                ...
                <Card sx={{ 
                  ...
                  <CardContent sx={{ p: 4 }}>
                    <QuestionDisplay
                      ...
                      disabled={!isBattleActive(battle_state)}
                    />
                  </CardContent>
                </Card>
              </motion.div>
            ...
            {/* Battle results */}
            {isBattleCompleted(battle_state) && (
              <>
            ...
            {/* Battle error */}
            {isBattleError(battle_state) && battle_state?.error && (
