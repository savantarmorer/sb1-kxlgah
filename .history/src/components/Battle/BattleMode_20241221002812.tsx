  ...
  return (
    <Box sx={{ 
                ...
                <CardContent>
                  ...
                  {isBattleActive && current_question && (
                    <>
                      <Box sx={{ mb: 4 }}>
                        <Timer
                          timeLeft={battle_state?.time_left || 0}
                          isRunning={isBattleActive}
                        />
                      </Box>
                      <QuestionDisplay
                        question={current_question}
                        handleAnswerSelection={answer_question}
                        eliminatedOptions={battle_state?.metadata?.eliminated_options}
                      />
                    </>
                  )}
                  ...
                  {isBattleCompleted && (
                    <BattleResultsComponent
                      battleScore={battle_state?.score || { player: 0, opponent: 0 }}
                      battleRewards={rewards}
                      handleClaim={handle_battle_completion}
                      isClaimed={rewards_claimed}
                    />
                  )}
                  ...
                  {isBattleError && (
                    <Box sx={{ textAlign: 'center', py: 4 }}>
                      <Typography color="error" gutterBottom>
                        {error?.message || t('battle.error.generic')}
                      </Typography>
                      <Button
                        variant="contained"
                        color="primary"
      ...
      {show_stats && (
        <Box sx={{ p: 2 }}>
          <ScoreDisplay
            playerScore={battle_state?.score?.player || 0}
            opponentScore={battle_state?.score?.opponent || 0}
            totalQuestions={battle_progress.total_questions}
          />
        </Box>
      )}
      ...
      <BattleStateTransition
        status={battle_state?.status || 'idle'}
        message={battle_state ? getBattleStatusMessage(battle_state.status) : ''}
        onComplete={() => updateState({ is_transitioning: false })}
      />
    </Box>
  );
}
