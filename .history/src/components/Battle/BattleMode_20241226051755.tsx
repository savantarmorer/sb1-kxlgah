...
import { BattlePhase, BattleStatus, BattleQuestion } from '../../types/battle';
...
export default function BattleMode({ mode = 'all' }: BattleModeProps) {
  ...
  const { timeLeft } = useTimer(BATTLE_CONFIG.time_per_question);
  ...
  }, [phase, selectedCard, playerHand, selectCard, setSelectionTimeLeft]);

  // Add a timer effect for questions
  useEffect(() => {
    if (phase !== BattlePhase.QUESTION) return;
    ...
    const timer = setInterval(() => {
      setTimeLeft((prev) => {
        if (prev <= 1) {
          ...
          handleAnswer('', selectedCard!, opponentSelectedCard!, playerHand, opponentHand);
          return 0;
        }
        return prev - 1;
      });
    ...
    return () => clearInterval(timer);
  }, [phase, currentQuestion, handleAnswer, selectedCard, opponentSelectedCard, playerHand, opponentHand, setTimeLeft]);
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
          ...
          {phase === BattlePhase.QUESTION && currentQuestion && (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4, p: 4 }}>
              ...
              <QuestionDisplay
                ...
                onAnswer={(answer) => handleAnswer(answer, selectedCard!, opponentSelectedCard!, playerHand, opponentHand)}
      ...
      <BattleFooter
        battleStats={{
          ...
          rating: state.battle_ratings?.rating || BATTLE_CONFIG.matchmaking.default_rating
        }}
        phase={phase}
      />
