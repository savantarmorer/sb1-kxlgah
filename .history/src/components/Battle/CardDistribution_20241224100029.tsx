...
const CARD_GAP = -30; // Negative gap for overlapping
const CARD_OVERLAP = 40; // How much cards overlap
...
export function CardDistribution({ playerCards, opponentCardCount, onComplete }: CardDistributionProps) {
  ...
  const totalWidth = (playerCards.length * (CARD_WIDTH - CARD_OVERLAP));
  const startX = -(totalWidth / 2) + (CARD_WIDTH / 2);

  const getCardPosition = (index: number) => ({
    x: startX + (index * (CARD_WIDTH - CARD_OVERLAP)),
    y: 0
  });
  ...
  const shuffleVariants = {
    ...
    deal: (i: number) => ({
      ...
      y: i < playerCards.length ? 300 : -300,
  ...
  React.useEffect(() => {
    ...
    const dealTimer = setTimeout(() => {
      onComplete();
    }, 2400 + ((playerCards.length + opponentCardCount) * 150) + 500);
        ...
        <>
          ...
          {Array.from({ length: opponentCardCount }).map((_, index) => (
            <motion.div
              ...
              transition={{
                duration: 0.8,
                delay: (index + playerCards.length) * 0.15 + 0.1,
                type: 'spring',
                stiffness: 150,
                damping: 12
              }}
