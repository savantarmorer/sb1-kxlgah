...
export function BattleItemDisplay({ items, onUseItem, disabled }: BattleItemDisplayProps) {
  ...
  const isBattleEffect = (effectType: ItemEffectType | string): boolean => {
    return [
  ...
  const getEffectDescription = (effect: { type: string; value?: number }): string => {
    switch (effect.type) {
  ...
  const battleItems = items.filter(item => {
    ...
    const hasBattleEffects = 
      ...
      (!Array.isArray(effects) && effects && typeof effects === 'object' && (
        ('type' in effects && typeof (effects as { type: string }).type === 'string' && isBattleEffect((effects as { type: string }).type)) ||
        ('boostType' in effects && typeof (effects as { boostType: string }).boostType === 'string' && (effects as { boostType: string }).boostType === 'battle') ||
        ('metadata' in effects && typeof (effects as { metadata: { battle_only?: boolean } }).metadata === 'object' && (effects as { metadata: { battle_only?: boolean } }).metadata?.battle_only) ||
        ('battle_only' in effects && typeof (effects as { battle_only: boolean }).battle_only === 'boolean' && (effects as { battle_only: boolean }).battle_only)
      ));
