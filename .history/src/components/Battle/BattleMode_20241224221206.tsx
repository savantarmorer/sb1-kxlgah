  ...
  const initializeBattle = async () => {
    try {
      ...
      if (!questionsData || questionsData.length === 0) {
        showError(`No questions available for ${selectedMode} mode`);
        setPhase(BATTLE_PHASES.ERROR);
        return;
      }
      ...
      setPlayerState({ health: 50, shield: 0 });
      setOpponentState({ health: 50, shield: 0 });
      ...
      setPhase(BATTLE_PHASES.SELECTION);
      setSelectionTimeLeft(10);
    } catch (error) {
      console.error('Error initializing battle:', error);
      showError(`Error initializing battle: ${error instanceof Error ? error.message : 'Unknown error'}`);
      setPhase(BATTLE_PHASES.ERROR);
    }
  };
