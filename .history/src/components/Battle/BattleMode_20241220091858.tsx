...
// Re-export BattlePhase as a value
const BattlePhases = {
  waiting: 'waiting',
  ready: 'ready',
  active: 'active',
...
interface ExtendedBattleState extends Omit<BattleState, 'error' | 'metadata'> {
  error?: string | null;
...
const castBattleState = (state: BattleState | null): ExtendedBattleState | null => {
  ...
  const { error, metadata, phase = BattlePhases.waiting, ...rest } = state;
  return {
    ...rest,
    phase: phase as BattlePhaseType,
...
export function BattleMode({ on_close }: BattleModeProps) {
  ...
  const battle_state = castBattleState(rawBattleState);
