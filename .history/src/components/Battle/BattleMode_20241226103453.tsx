...
export default function BattleMode({ mode = 'all', difficulty = 'medium', is_bot = true }: BattleModeProps) {
  ...
  useEffect(() => {
    ...
    if (phase === BattleStateEnum.CARD_SELECTION && selectedCard) {
      ...
      if (playerCard) {
        ...
        }, 500); // Give time for opponent card selection to complete
  ...
  }, [phase, selectedCard, playerHand, selectOpponentCard, setPhase, setCurrentQuestion, state.battle?.current_question, state.battle?.questions, showError, t]);
  ...
  const handleBattleStart = useCallback(async () => {
    try {
      ...
      // First set the state to initializing
      ...
      // Move to dealing phase first
      setPhase(BattleStateEnum.DEALING);
  ...
  }, [initializeBattle, initializeDecks, difficulty, is_bot, mode, showError, t, dispatch, setPhase]);
  ...
  const handleDistributionComplete = useCallback(async () => {
    try {
      ...
      await setPhase(BattleStateEnum.CARD_SELECTION);
    } catch (error) {
  ...
  }, [distributeCards, setPhase, showError, t]);
