...
export default function BattleMode({ mode = 'all' }: BattleModeProps) {
  ...
  const handleCardSelect = (cardId: string) => {
    if (selectedCard || phase as string !== BattlePhase.CARD_SELECTION) return;
    ...
    setTimeout(() => {
      if (phase as string === BattlePhase.CARD_REVEAL) {
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Box sx={{ 
        ...
        <AnimatePresence mode="wait">
          ...
          {([BattlePhase.CARD_SELECTION, BattlePhase.CARD_REVEAL] as string[]).includes(phase as string) && (
            <Box sx={{ 
              display: 'flex', 
          ...
          {phase as string === BattlePhase.QUESTION && currentQuestion && (
          ...
          {phase as string === BattlePhase.COMPLETED && (
            <BattleResults
              score={score}
              streak={streak}
