...
export function BattleMode({ on_close }: BattleModeProps) {
  ...
  const handleUseItem = async (item: InventoryItem) => {
    ...
    try {
      // Update the item quantity in the database first
      ...
      if (error) {
        console.error('[BattleMode] Error updating item quantity:', error);
        throw error;
      }

      // Update the item quantity in the state
      dispatch({
      ...
      console.log('[BattleMode] Item quantity updated');
          ...
          switch (effect.type) {
            case 'eliminate_wrong_answer':
              ...
              if (currentQuestion) {
                ...
                console.log('[BattleMode] Elimination attempt:', {
                  correctAnswer,
                  currentlyEliminated: currentQuestion.eliminated_options || []
                });
                
                // Get all wrong alternatives that haven't been eliminated yet
                const wrongAnswers = [
                  ...
                  alt.key.toLowerCase() !== correctAnswer.toLowerCase() &&
                  // Must not be already eliminated
                  !currentQuestion.eliminated_options?.includes(alt.key)
                );
