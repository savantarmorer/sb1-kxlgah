...
export default function BattleMode({ mode = 'all' }: BattleModeProps) {
  ...
  const handleReady = () => {
    ...
    setTimeout(() => {
      ...
      const botAnswer = botCorrect ? currentQuestion?.correct_answer || 'A' : ['A', 'B', 'C', 'D'].filter(a => a !== currentQuestion?.correct_answer)[Math.floor(Math.random() * 3)];
      ...
      // Process battle round after bot's turn
      setTimeout(() => {
        processBattleRound();
      }, 1000);
    }, 1000);
