...
import { 
  ...
  BattleResults,
  BattleMetadata as BattleMetadataType,
  EnhancedBattleState as EnhancedBattleStateType,
  BattlePlayerState as BattlePlayerStateType,
  BattleError,
  BattlePhase
} from '../../types/battle';
...
/**
 * Core Battle Interfaces
 * These interfaces define the structure of battle-related data
 */

/**
 * BattleScore Interface
 * Tracks the current score state for both players
 * 
 * Used by:
...
/**
 * BattlePlayerState Interface
 * Tracks individual player state during battle
 * 
 * Used by:
...
/**
 * BattleModeState Interface
 * Internal state management for BattleMode component
 */
...
/**
 * PreBattleState Interface
 * Manages the pre-battle countdown and ready states
 */
 ...
 * These functions handle battle state checks and transformations
 */

/**
 * Checks if the current battle is a bot battle
 * 
 * @param state - The current battle state
 * @returns boolean indicating if it's a bot battle
 * 
 * Used by:
...
/**
 * Handles error messages in a consistent format
 * 
...
/**
 * Safely casts battle state to extended type
 * 
 * @param state - The battle state to cast
 * @returns Properly typed battle state
 * 
 * Used by:
 * - State management
 * - Component updates
...
// Helper function to map rewards
...
// Helper function to create enhanced battle state
const createEnhancedBattleState = (
  battle_state: ExtendedBattleState & { rewards: BattleRewards },
  battle_progress: { total_questions: number; score: BattleScoreType }
): EnhancedBattleStateType => {
  const defaultMetadata: Required<BattleMetadataType> = {
    is_bot: false,
    questions: [],
    current_question: 0,
    player_states: {} as Record<string, BattlePlayerStateType>,
    started_at: new Date().toISOString(),
  ...
  const metadata: Required<BattleMetadataType> = {
    ...defaultMetadata,
    ...battle_state.metadata as Required<BattleMetadataType>,
    phase: 'completed' as BattlePhase,
    total_questions: battle_progress.total_questions,
    time_per_question: BATTLE_CONFIG.time_per_question
  };
  ...
  const battleId = battle_state.matchId || crypto.randomUUID();
  const startTime = Date.now();

  const enhancedState: EnhancedBattleStateType = {
    ...battle_state,
    id: battleId,
    start_time: startTime,
    user: {
      id: Object.keys(battle_state.metadata?.player_states || {})[0] || '',
      xp: 0,
      streak: 0
    },
    time_per_question: BATTLE_CONFIG.time_per_question,
    ...
    status: battle_state.status as BattleStatus,
...
interface BattleStateWithMetadata extends ExtendedBattleState {
...
/**
 * Handles battle completion and reward distribution
 * 
...
const handleBattleCompletion = useCallback(async () => {
  ...
  try {
    ...
    const metadata: Required<BattleMetadataType> = {
      ...
      player_states: battle_state.metadata?.player_states || {} as Record<string, BattlePlayerStateType>,
 ...
 * 
 * @param selected_answer - The answer selected by the player
 * @returns Promise<void>
 * 
 * Dependencies:
 * - battle_state
...
export function BattleMode({ on_close }: BattleModeProps) {
  ...
  const renderPreBattleModal = () => {
    if (!state.battle?.opponent || !state.user || state.battle.status !== 'ready') return null;
   ...
   * These constants and functions ensure type safety for battle statuses
   */

  const validBattleStatuses: BattleStatus[] = [
    'idle',
    'waiting',
    'preparing',
    'searching',
  ...
  /**
   * Type guard for battle status validation
   * 
   * @param status - Status to validate
   * @returns Type predicate for BattleStatus
   */
  const isBattleStatus = (status: string): status is BattleStatus => {
    return validBattleStatuses.includes(status as BattleStatus);
  };

  /**
   * Battle status checks for component rendering
   */
  const battleStatus = {
    isActive: battle_state?.status === 'active' && isBattleStatus(battle_state.status),
    isCompleted: battle_state?.status === 'completed' && isBattleStatus(battle_state.status),
    isError: battle_state?.status === 'error' && isBattleStatus(battle_state.status),
    isWaiting: battle_state?.status === 'waiting' && isBattleStatus(battle_state.status),
    isPreparing: battle_state?.status === 'preparing' && isBattleStatus(battle_state.status),
    isPaused: battle_state?.status === 'paused' && isBattleStatus(battle_state.status)
  };
