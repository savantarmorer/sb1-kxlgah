...
export default function BattleMode({ mode = 'all' }: BattleModeProps) {
  ...
  const handleReady = () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        console.log('Processing battle round');
  ...
  const processBattleRound = async () => {
    console.log('processBattleRound started', { playerState, opponentState });
    if (!playerState.selectedAction || !opponentState.selectedAction) {
      console.log('Missing actions', { 
        playerAction: playerState.selectedAction, 
        opponentAction: opponentState.selectedAction 
      });
      return;
    }
