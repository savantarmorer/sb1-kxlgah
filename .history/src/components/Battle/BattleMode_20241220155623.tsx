  ...
  return {
    ...
    metadata: {
      ...
      multiplayer: {
        ...
        opponent_connection_status: 'connected',
        last_sync_timestamp: Date.now(),
        sync_attempts: 0
      },
      match_id: state.id,
      round_number: 1,
      time_remaining: state.metadata.time_per_question || 30,
      sync_token: `${state.id}_${Date.now()}`
    },
    network_state: {
      is_host: true,
      peer_id: state.metadata.player_states[state.user?.id || '']?.id || '',
      connected_peers: [],
      last_ping: Date.now()
    }
  };
