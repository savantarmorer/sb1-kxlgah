...
import styles from './BattleStateTransition.module.css';
...
export function BattleStateTransition({
  ...
  const player: PlayerPreview = {
    ...
    rating: Number(state.battle_stats?.tournament_rating || 1000),
  ...
  return (
    <div className={styles.battleTransition}>
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        exit={{ opacity: 0, y: -20 }}
      ...
      >
        {/* Battle Preview */}
        {status === 'preparing' && (
          <div className={styles.battleTransitionContent}>
