...
import { BattleLobbyBotSetup } from './BattleLobbyBotSetup';
import { BattleMode } from './BattleMode';
import { BattleToast } from '../notifications/toasts/BattleToast';
...
export function BattleLobby({ on_start_battle, on_close, stats }: BattleLobbyProps) {
  ...
  const { joinQueue, matchmakingState } = useMatchmaking();
  ...
  }, [matchmakingState.status, joinQueue]);
  ...
  return (
    <Container maxWidth="lg">
      ...
      >
        ...
        {/* Button to start against a bot */}
        <Button onClick={() => setUseBot(true)} disabled={useBot}>
          Play Against Bot
        </Button>
      </motion.div>

      {/* If useBot is true, redirect to bot battle setup */}
      {useBot && (
        <BattleLobbyBotSetup match_id={`bot_${Date.now()}`} />
      )}

      <BattleToast visible={toastVisible} result={battleResult} />
      {battleStarted && <BattleMode />}
    </Container>
  );
} 