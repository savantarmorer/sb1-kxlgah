...
import { Box, Typography, Card, CardContent, Button, Modal, ModalOverlay, ModalContent, ModalHeader, ModalBody, VStack, HStack, Text, Progress, Stack, LinearProgress } from '@mui/material';
...
// Update error interface to match usage
interface BattleError extends Error {
  timestamp: number;
}

// Update BattleState interface to include missing properties
...
// Update the type assertion for battle_state
const battle_state = useBattle().battle as ExtendedBattleState;
...
export function BattleMode({ on_close }: BattleModeProps) {
  ...
  const isBotBattle = (state: ExtendedBattleState): boolean => {
  ...
  useEffect(() => {
    if (battle_error) {
      const errorMessage = battle_error instanceof Error ? battle_error.message : String(battle_error);
