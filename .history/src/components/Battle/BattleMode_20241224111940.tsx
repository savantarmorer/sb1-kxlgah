...
export default function BattleMode({ mode = 'all' }: BattleModeProps) {
  ...
  const handleCardSelect = useCallback((cardId: string) => {
    ...
    console.log('[BattleMode] Cards selected:', { player: playerCard, opponent: opponentCard });
  ...
  }, [selectedCard, phase, playerHand, opponentHand, opponentState.health]);
  ...
  const handleAttackAnimationComplete = useCallback(() => {
    console.log('[BattleMode] Attack animation completed. Transitioning to next phase.');
    setPhase(BATTLE_PHASES.QUESTION);
    setSelectedCard(undefined);
    setOpponentSelectedCard(undefined);
  }, []);
  ...
  const drawInitialHands = useCallback(() => {
    const playerInitialHand = drawCards(playerDeck, INITIAL_HAND_SIZE);
    const opponentInitialHand = drawCards(opponentDeck, INITIAL_HAND_SIZE);
    
    setPlayerHand(playerInitialHand);
    setOpponentHand(opponentInitialHand);
    
    console.log('[BattleMode] Initial hands drawn:', { playerInitialHand, opponentInitialHand });
  }, [playerDeck, opponentDeck]);

  useEffect(() => {
    if (phase === BATTLE_PHASES.PREPARING) {
      const deck = createDeck();
      setPlayerDeck(deck);
      setOpponentDeck(createDeck());
      setPhase(BATTLE_PHASES.DEALING);
      console.log('[BattleMode] Decks created. Moving to DEALING phase.');
    }
  }, [phase]);

  useEffect(() => {
    if (phase === BATTLE_PHASES.DEALING) {
      drawInitialHands();
      setPhase(BATTLE_PHASES.CARD_SELECTION);
      console.log('[BattleMode] Cards dealt. Moving to CARD_SELECTION phase.');
    }
  }, [phase, drawInitialHands]);
  ...
  }, [playerState.health, opponentState.health, score, streak, timeLeft, handle_battle_completion, showError]);
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Box sx={{ 
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'DEALING' && (
            <motion.div
              variants={animateCardDistribution}
              initial="hidden"
              animate="visible"
              exit="hidden"
            >
              <CardDistribution
                ...
                onComplete={() => setPhase(BATTLE_PHASES.CARD_SELECTION)}
              />
            </motion.div>
          )}
