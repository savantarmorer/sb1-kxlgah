...
export type GameAction = 
  ...
  | { type: 'UPDATE_BATTLE_MODE'; payload: string }
...
export default function BattleMode({ mode = 'all', difficulty = 'medium', is_bot = true }: BattleModeProps) {
  ...
  }, [initializeBattle, difficulty, is_bot, mode, showError, t, dispatch]);

  // Debug render - only log when phase changes
  useEffect(() => {
    console.debug('[BattleMode] Phase changed:', { 
      phase, 
      currentQuestion, 
      playerState, 
      opponentState, 
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
          ...
          {phase === BattleStateEnum.PREPARING && (
            <PreBattleLobby
              ...
              onModeSelect={(newMode) => {
                // Don't change phase here, just update the mode
                if (newMode !== mode) {
                  dispatch({ type: 'UPDATE_BATTLE_MODE', payload: newMode });
                }
              }}
              selectedMode={mode}
            />
      ...
      {error && <BattleErrorBoundary message={error} />}
    </Container>
  );
}