...
import { 
  ...
  Timer, Medal, TrendingUp, BookOpen, Scale, Gavel, FileText, Loader2 
} from 'lucide-react';
...
function PreBattleModal({ player, opponent, timeLeft, onComplete }: PreBattleModalProps) {
  useEffect(() => {
    if (timeLeft === 0) {
      onComplete();
    }
  ...
  const getRankTier = (rating: number) => {
    if (rating >= 2000) return { name: 'Master', color: 'text-purple-500' };
    if (rating >= 1800) return { name: 'Diamond', color: 'text-blue-400' };
    if (rating >= 1600) return { name: 'Platinum', color: 'text-cyan-500' };
    if (rating >= 1400) return { name: 'Gold', color: 'text-yellow-500' };
    if (rating >= 1200) return { name: 'Silver', color: 'text-gray-400' };
    return { name: 'Bronze', color: 'text-orange-500' };
  };

  const playerRank = getRankTier(player.rating);
  const opponentRank = getRankTier(opponent.rating);
  ...
  return (
    ...
    >
      <motion.div 
        ...
        className="bg-gray-900 border border-gray-800 rounded-lg p-8 max-w-2xl w-full mx-4"
      >
        <div className="text-center mb-6">
          <h2 className="text-2xl font-bold text-brand-teal-500 mb-2">Battle Starting</h2>
          <div className="text-5xl font-bold text-yellow-500">{timeLeft}</div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 items-center">
          ...
          <div className="text-center">
            <div className="relative inline-block">
              <img 
                ...
                alt={player.name}
                className="w-24 h-24 rounded-full border-4 border-brand-teal-500"
              />
              <div className="absolute -bottom-2 -right-2 bg-gray-900 rounded-full p-1 border-2 border-brand-teal-500">
                <Trophy className="w-6 h-6 text-brand-teal-500" />
              </div>
            </div>
            <h3 className="text-xl font-bold mt-2 text-white">{player.name}</h3>
            <div className={`text-sm font-semibold ${playerRank.color}`}>
              {playerRank.name}
            </div>
          ...
          <div className="text-center">
            ...
            <div className="text-xl font-bold text-red-500 mt-2">VS</div>
          </div>
          ...
          <div className="text-center">
            <div className="relative inline-block">
              ...
              />
              <div className="absolute -bottom-2 -right-2 bg-gray-900 rounded-full p-1 border-2 border-red-500">
                <Crown className="w-6 h-6 text-red-500" />
              </div>
            </div>
            <h3 className="text-xl font-bold mt-2 text-white">{opponent.name}</h3>
            <div className={`text-sm font-semibold ${opponentRank.color}`}>
              {opponentRank.name}
            </div>
            <div className="text-sm text-gray-400">
              Level {opponent.level}
            </div>
            {opponent.streak && opponent.streak > 0 && (
              <div className="text-sm text-orange-500">
                {opponent.streak} Win Streak 🔥
              </div>
            )}
          </div>
        </div>
...
export function BattleLobby({ onStartBattle, onClose, stats }: BattleLobbyProps) {
  ...
  useEffect(() => {
    if (isMatched && opponent) {
      onStartBattle();
    }
  }, [isMatched, opponent, onStartBattle]);

  // Handle match found
  useEffect(() => {
    if (isMatched && opponent && state.user) {
      setShowPreBattle(true);
      setCountdown(5);
      
      // Don't start battle until countdown is complete
      if (countdown > 0) {
      ...
      } else if (countdown === 0 && showPreBattle) {
        setShowPreBattle(false);
        onStartBattle();
      }
    }
  }, [isMatched, opponent, state.user, countdown, showPreBattle]);

  // Separate effect for starting battle after countdown
  useEffect(() => {
    if (countdown === 0 && showPreBattle) {
      setShowPreBattle(false);
      onStartBattle();
    }
  }, [countdown, showPreBattle]);
  ...
  return (
    <motion.div className="relative">
        ...
        <div className="grid grid-cols-1 gap-4">
          {isSearching ? (
            <Button
              ...
              variant="outline"
              className="w-full flex items-center justify-center gap-2 py-3 border-red-500 text-red-500 hover:bg-red-50"
            >
            ...
            <Button
              onClick={selectedMode === 'ranked' ? handleStartMatchmaking : onStartBattle}
              variant="primary"
              ...
              disabled={selectedCategory !== 'general' || (selectedMode === 'ranked' && !state.user)}
            >
              <Swords className="w-5 h-5" />
              {selectedMode === 'ranked' 
                ? (selectedCategory === 'general' ? 'Find Match' : 'Coming Soon')
      ...
      <AnimatePresence>
        {showPreBattle && state.user && opponent && (
          <PreBattleModal
            player={{
