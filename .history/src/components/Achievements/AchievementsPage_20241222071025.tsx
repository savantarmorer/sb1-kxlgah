...
import { Trophy, Settings, Gift, Palette } from 'lucide-react';
...
import LootboxReward from '../Lootbox/LootboxReward';
...
export function AchievementsPage() {
  ...
  // Test rewards for the lootbox
  const testRewards = [
    { type: 'xp', value: 100, rarity: 'common' as const },
    { type: 'coins', value: 500, rarity: 'rare' as const },
    { type: 'item', value: 'Legendary Trophy', rarity: 'legendary' as const }
  ];
      ...
      >
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between gap-3 mb-4">
            ...
            {/* Admin buttons with debug info */}
            <div className="flex items-center gap-2">
              <button
                onClick={() => setShowLootbox(true)}
                className="flex items-center gap-2 px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg transition-colors shadow-sm"
              >
                <Gift size={20} />
                <span className="font-medium">Test Lootbox</span>
              </button>
              {(user?.is_super_admin || process.env.NODE_ENV === 'development') && (
                <>
                  <button
                    onClick={() => navigate('/achievements/editor')}
                  ...
                  <button
                    onClick={() => {
                      console.log('Admin button clicked', { is_super_admin: user?.is_super_admin });
                  ...
                  >
                    ...
                    <span className="font-medium">
                      Admin Dashboard
                      {!user?.is_super_admin && process.env.NODE_ENV === 'development' && ' (Dev Mode)'}
                    </span>
                  </button>
                </>
              )}
