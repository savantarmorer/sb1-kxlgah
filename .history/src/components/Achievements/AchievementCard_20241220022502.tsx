...
export function AchievementCard({ achievement, userId, onUpdate }: { 
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      if (success) {
        ...
        toast({
          title: 'Error',
          description: 'Failed to claim achievement rewards',
          status: 'error',
          duration: 3000,
          isClosable: true,
        });
  ...
  const renderRewards = () => {
    ...
    if (achievement.rewards) {
      rewards.push(...achievement.rewards.map((reward, index) => (
        <Text key={`reward-${index}`} fontSize="sm" color="gray.500">
          {reward.type === 'coins' && '🪙'}
          {reward.type === 'gems' && '💎'}
          {reward.type === 'xp' && '⭐'}
          {reward.type === 'title' && '📜'}
  ...
  return (
    ...
    >
      ...
      {nextMilestone && !achievement.unlocked && (
        <div className="mt-4 border-t border-gray-200/10 pt-4">
