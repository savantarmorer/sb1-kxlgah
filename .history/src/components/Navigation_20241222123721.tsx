...
const navItems: NavItem[] = [
  { label: 'Inventory', path: '/inventory', icon: Package, view: View.INVENTORY },
  { label: 'Battle', path: '/battle', icon: Sword, view: View.BATTLE }
];
...
export default function Navigation() {
  ...
  const fetchUserTitle = async () => {
    ...
    try {
      ...
      if (titleData?.display_titles) {
        setUserTitle(titleData.display_titles.name);
        setUserTitleRarity(titleData.display_titles.rarity);
      } else {
...
export default function Navigation({ currentView, onViewChange, isAdmin = false }: NavigationProps) {
  ...
  const [userTitle, setUserTitle] = useState<string>('Adventurer');

  // Add effect to fetch user's equipped title
  const fetchUserTitle = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .eq('user_id', state.user.id)
        .eq('is_equipped', true)
        .maybeSingle() as { data: TitleData | null; error: any };
  ...
  useEffect(() => {
    ...
    const titleSubscription = supabase
      .channel('user_title_changes')
      .on(
        'postgres_changes',
        {
          event: '*',
          schema: 'public',
          table: 'user_display_titles',
          filter: `user_id=eq.${state.user.id}`,
        },
        () => {
          // Refetch title when changes occur
          fetchUserTitle();
        }
      )
      .subscribe();
    ...
    return () => {
      supabase.removeChannel(titleSubscription);
    };
  }, [state.user?.id, fetchUserTitle]);
            ...
            >
              ...
              >
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 font-medium">
                  Level {state.user?.level || 1} • {userTitle}
                </p>
              </motion.div>
            </motion.div>
            ...
            {navItems.map(item => (
              <button
                ...
                className={`
                  ...
                  ${currentView === item.id ? 'text-primary-500' : 'hover:text-gray-100'}
                `}
              >
