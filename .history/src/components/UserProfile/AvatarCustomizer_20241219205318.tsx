...
export default function AvatarCustomizer() {
  ...
  const [previewImage, setPreviewImage] = useState(state.user?.avatar_url || '/default-avatar.png');
        ...
        <div className="flex items-center space-x-4">
          <img
            ...
            className={`w-24 h-24 rounded-full object-cover border-4 ${
              AVATAR_FRAMES.find(f => f.id === state.user?.avatar_frame)?.borderColor || AVATAR_FRAMES[0].borderColor
            }`}
          />
          <div>
            <h4 className="font-medium text-gray-900 dark:text-white">{state.user?.name}</h4>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {AVATAR_FRAMES.find(f => f.id === state.user?.avatar_frame)?.name || 'Default'} Frame
            </p>
          </div>
        </div>
      )}
