  ...
  const loadUserInventory = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .not('item.metadata->isTitle', 'eq', true);
      ...
      const transformedItems = data
        ?.filter(record => record.item !== null)
        .map(record => ({
          id: record.item.id,
      ...
      const nullItems = data?.filter(record => record.item === null);
      if (nullItems?.length) {
        console.warn('Found inventory records with null items:', nullItems);
      }
