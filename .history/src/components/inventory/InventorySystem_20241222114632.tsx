  ...
  const loadUserInventory = async () => {
    ...
    try {
      ...
      const transformedCosmeticItems = cosmeticItems?.map(record => ({
        id: record.items.id,
        name: record.display_titles?.name || record.items.name,
        description: record.display_titles?.description || record.items.description,
        type: record.items.type,
        rarity: record.display_titles?.rarity || record.items.rarity,
        equipped: record.equipped,
        quantity: record.quantity,
        effects: record.items.effects,
        metadata: { ...record.items.metadata, ...(record.display_titles?.metadata || {}) },
        icon: record.items.icon,
        icon_color: record.items.icon_color,
