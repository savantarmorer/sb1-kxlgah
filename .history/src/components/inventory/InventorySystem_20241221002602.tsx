  ...
  const loadUserInventory = async () => {
    ...
    try {
      ...
      const transformedItems = data?.map(record => ({
        ...
        effects: Array.isArray(record.item.effects) 
          ? record.item.effects.map(effect => ({
              ...effect,
              metadata: {
                ...effect.metadata,
        ...
        metadata: {
          ...
          battle_only: Array.isArray(record.item.effects) && 
            record.item.effects.some(effect => effect.type === 'eliminate_wrong_answer')
        },
        icon: record.item.icon,
        icon_color: record.item.icon_color,
        acquired_at: record.acquired_at,
        imageUrl: record.item.metadata?.icon || 
          (Array.isArray(record.item.effects) && 
           record.item.effects.some(effect => effect.type === 'eliminate_wrong_answer')
            ? `/images/items/${record.item.id}.png`
            : `/items/${record.item.type.toLowerCase()}.png`),
        is_active: true
      })) || [];
