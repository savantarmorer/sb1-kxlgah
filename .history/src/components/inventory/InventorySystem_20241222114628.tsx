  ...
  const loadUserInventory = async () => {
    ...
    try {
      ...
      const { data: regularItems, error: regularError } = await supabase
        ...
        .neq('item:items.type', ItemType.COSMETIC);

      if (regularError) throw regularError;

      // Then load cosmetic items (titles) with their display_titles data
      const { data: cosmeticItems, error: cosmeticError } = await supabase
        ...
        .select(`
          *,
          ...
          display_title:display_titles!inner (
            id,
            name,
            description,
            rarity,
        ...
        `)
        .eq('user_id', state.user.id)
        .eq('item:items.type', ItemType.COSMETIC);
      ...
      const transformedCosmeticItems = cosmeticItems?.map(record => ({
        id: record.item.id,
        name: record.display_title.name || record.item.name,
        description: record.display_title.description || record.item.description,
        type: record.item.type,
        rarity: record.display_title.rarity || record.item.rarity,
        equipped: record.equipped,
        quantity: record.quantity,
        effects: record.item.effects,
        metadata: { ...record.item.metadata, ...record.display_title.metadata },
        icon: record.item.icon,
        icon_color: record.item.icon_color,
        imageUrl: record.item.icon || `/images/items/title.png`,
        acquired_at: record.acquired_at,
        isTitle: true
      })) || [];
