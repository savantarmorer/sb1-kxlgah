...
export default function InventorySystem() {
  ...
  const loadUserInventory = async () => {
    ...
    try {
      ...
      // Then load cosmetic items (titles) with their equipped status from user_display_titles
      ...
      const transformedCosmeticItems = cosmeticItems?.filter(record => record.items).map(record => ({
        ...
        type: record.items.type,
        rarity: record.items.rarity,
        equipped: record.user_display_titles?.[0]?.is_equipped || false,
        quantity: record.quantity,
        effects: record.items.effects,
        metadata: record.items.metadata,
        icon: record.items.icon,
  ...
  const filteredItems = useMemo(() => {
    ...
    // Title tab - show only titles (items with type COSMETIC)
    if (showTitles) {
      return userItems.filter(item => item.type === ItemType.COSMETIC || item.isTitle);
    }

    // Regular items tab - show everything except titles and avatars
    return userItems.filter(item => 
      item.type !== ItemType.COSMETIC && 
