...
export default function Navigation({ currentView, onViewChange, isAdmin }: NavigationProps) {
  ...
  const fetchUserTitle = async () => {
    ...
    try {
      ...
      if (titleData && typeof titleData.title_id === 'number' && 
          typeof titleData.is_equipped === 'boolean' && 
          titleData.display_titles && 
          typeof titleData.display_titles.name === 'string' && 
          typeof titleData.display_titles.rarity === 'string') {
        const userDisplayTitle = titleData as TitleData;
        setUserTitle(userDisplayTitle.display_titles.name);
        setUserTitleRarity(userDisplayTitle.display_titles.rarity);
      } else {
  ...
  const handleLogout = async () => {
    try {
      ...
      dispatch({ type: 'INITIALIZE_USER', payload: null });
      navigate('/login');
    } catch (error) {
  ...
  const user = state.user ? (state.user as unknown as SupabaseUser) : null;
