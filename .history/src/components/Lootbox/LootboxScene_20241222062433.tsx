...
function LootboxModel({ 
  ...
  if (state === 'IDLE') {
    ...
    const exploded = useOpeningAnimation(boxRef, lidRef);
  ...
  const CenterEmblem = () => (
    <group position={[0, BOX_DIMENSIONS.lidHeight/2 + 0.01, 0]}>
      ...
      <mesh position={[0, 0.02, 0]} scale={[0.2, 0.02, 0.2]} rotation={[0, Math.PI / 8, 0]}>
        ...
        <group ref={boxRef}>
          ...
          {[...Array(4)].map((_, i) => (
            <group key={`border-${i}`}>
              ...
              <RoundedBox
                args={[
                  BOX_DIMENSIONS.borderThickness,
          ...
          {[...Array(2)].map((_, i) => (
            <RoundedBox
              key={`band-${i}`}
              args={[BOX_DIMENSIONS.width + 0.02, 0.05, BOX_DIMENSIONS.depth + 0.02]}
              radius={0.01}
              smoothness={2}
