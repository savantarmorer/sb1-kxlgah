...
export const LootboxScene: React.FC<LootboxProps> = (props) => {
  ...
  // Force camera update when settings change
  const handleSettingsChange = (newSettings: SceneSettings) => {
    setSettings(newSettings);
    setKey(prev => prev + 1); // Force re-render of Canvas
  };

  return (
    <div className="w-full h-full relative">
      <SettingsEditor settings={settings} onSettingsChange={handleSettingsChange} />
      <Canvas 
        key={key} // Force re-render when camera settings change
        camera={{ 
