...
// Add this new component for smooth camera controls
const CameraController: React.FC<{ settings: SceneSettings }> = ({ settings }) => {
  const { camera } = useThree();
  
  useFrame(() => {
    camera.position.lerp(new THREE.Vector3(...settings.camera.position), 0.1);
    camera.fov = THREE.MathUtils.lerp(camera.fov, settings.camera.fov, 0.1);
...
const LootboxModel: React.FC<LootboxProps & { settings: SceneSettings }> = ({ rarity, isOpening, onOpenComplete, settings }) => {
  ...
  const targetPosition = useMemo(() => new THREE.Vector3(...settings.model.position), [settings.model.position]);
  const targetScale = useMemo(() => new THREE.Vector3(settings.model.scale, settings.model.scale, settings.model.scale), [settings.model.scale]);
  
  // Smooth position and scale updates
  useFrame(() => {
    if (group.current) {
  ...
  }, [settings.animation.spinStartSpeed, settings.animation.spinEndSpeed, settings.animation.duration]);
  ...
  // Update color immediately when settings change
  useEffect(() => {
    if (isOpening) {
      if (rarity === 'legendary') {
        setGlowColor(settings.legendarySequence[sequenceIndex]);
      } else {
        setGlowColor(settings.colors[rarity as keyof typeof settings.colors] || settings.colors.common);
      }
    }
  }, [settings.colors, settings.legendarySequence, rarity, isOpening, sequenceIndex]);
  ...
  }, [rarity, isOpening, settings.legendarySequence]);
  
  // Reset and update animations when settings change
  useEffect(() => {
    if (isOpening) {
  ...
  }, [isOpening, actions, names, onOpenComplete, rarity, settings.animation]);
