  ...
  const getImageUrl = (imageUrl: string | undefined, itemId: string): string => {
    ...
    if (Array.isArray(item.effects)) {
      ...
      if (iconFromEffects) {
        ...
        if (typeof iconFromEffects === 'string' && iconFromEffects.startsWith('custom:')) {
          return iconFromEffects.replace('custom:', '');
        }
        return typeof iconFromEffects === 'string' ? iconFromEffects : `/images/items/${itemId}.png`;
      }
    }

    // Get the image URL from either imageUrl or icon property
    const finalImageUrl = imageUrl || item.icon;

    // Then try the provided imageUrl
    if (!finalImageUrl) {
      console.log('[ItemIcon] No icon provided, using default:', `/images/items/${itemId}.png`);
  ...
  return (
    ...
    >
      {(item.imageUrl || item.icon) ? (
        <img
          src={getImageUrl(item.imageUrl || item.icon, item.id)}
          alt={item.name}
          onError={handleError}
