...
export default function Navigation({ currentView, onViewChange, isAdmin = false }: NavigationProps) {
  ...
  const [userTitleRarity, setUserTitleRarity] = useState<string>('common');

  useEffect(() => {
    if (state.user) {
      fetchUserTitle();

      // Subscribe to changes in user_display_titles
      const titleSubscription = supabase
        .channel('user_display_titles_changes')
        .on(
          'postgres_changes',
          {
            event: '*',
  ...
  const fetchUserTitle = async () => {
    ...
    try {
      ...
      const response = titleData as TitleResponse;
      if (response?.display_titles?.[0]) {
        setUserTitle(response.display_titles[0].name);
        setUserTitleRarity(response.display_titles[0].rarity.toLowerCase());
      } else {
