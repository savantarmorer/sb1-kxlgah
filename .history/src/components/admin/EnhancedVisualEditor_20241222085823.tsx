...
import { Code, Save, Eye, EyeOff, Grid as GridIcon, Settings, X, Keyboard, Plus, Link, Layers, Move, Play, Zap, Undo2, Redo2, Search, Command, Sun, Moon, RotateCw, AlignLeft, AlignCenter, AlignRight, AlignJustify, AlignHorizontalJustifyCenter, AlignVerticalJustifyCenter, Group, Ungroup, Lock, Unlock, ArrowUpToLine, ArrowDownToLine, ArrowUp, ArrowDown, List, Copy } from 'lucide-react';
...
export function EnhancedVisualEditor() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Duplicate component with Ctrl/Cmd + D
  ...
  const duplicateComponent = (component: EditableComponent) => {
  ...
  // Add keyboard shortcut for duplication
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (!isActive) return;
