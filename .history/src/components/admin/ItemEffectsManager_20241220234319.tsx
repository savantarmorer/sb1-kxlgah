      ...
      <Dialog open={showDialog} onClose={() => setShowDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 1 }}>
            ...
            {['instant_xp', 'instant_coins', 'battle_boost', 'eliminate_wrong_answer'].includes(currentEffect.type) && (
              <Grid item xs={12}>
                <TextField
                  fullWidth
                  type="number"
                  label={currentEffect.type === 'eliminate_wrong_answer' ? 'Wrong Answers to Eliminate' : 'Value'}
                  value={currentEffect.value}
                  onChange={(e) => handleEffectChange('value', e.target.value)}
                  InputProps={{ 
                    inputProps: { 
                      min: currentEffect.type === 'eliminate_wrong_answer' ? 1 : 0,
                      max: currentEffect.type === 'eliminate_wrong_answer' ? 2 : undefined
                    } 
                  }}
                />
              </Grid>
