...
import { Achievement, TriggerCondition, AchievementTriggerType, AchievementReward, AchievementMilestone } from '../../types/achievements';
  ...
  React.useEffect(() => {
    if (achievement) {
      ...
      if (achievement.rewards) {
        setRewards(achievement.rewards.map(r => ({
          type: r.type,
          value: r.value
        })));
      }
      if (achievement.milestones) {
        setMilestones(achievement.milestones.map(m => ({
          progress: m.progress,
        ...
        <form id="achievement-form" onSubmit={handleSubmit}>
          <Stack spacing={2}>
            ...
            <Box>
              ...
              {(formData.trigger_conditions || []).map((trigger: TriggerState, index: number) => (
                <Box
                  key={index}
                  sx={{
                    p: 2,
