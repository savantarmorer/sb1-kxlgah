...
interface EditableComponent {
  ...
  events: { [key: string]: string }; // Event name -> function name
...
export function EnhancedVisualEditor() {
  ...
  const createComponent = (template: ComponentTemplate) => {
    const element = document.createElement(template.type === 'button' ? 'button' : 'div');
  ...
  const addEventHandler = (component: EditableComponent, eventName: string, code: string) => {
    const functionName = `${component.id}_${eventName}`;
    setEditableComponents(prev => prev.map(c => {
      if (c.id === component.id) {
        return {
          ...c,
          events: { ...c.events, [eventName]: functionName },
          customFunctions: { ...c.customFunctions, [functionName]: code }
        };
      }
      return c;
    }));
    ...
    component.element[eventName] = new Function(code);
  };
