...
export default function TitleManager({ onClose }: TitleManagerProps) {
    ...
    try {
      ...
      const { error: itemError } = await supabase
        ...
        .update({
          ...
          cost: parseInt(formData.get('price') as string, 10),
          requirements: {
        ...
        .eq('id', selectedTitle.metadata?.item_id);
      ...
      const titleData = {
        ...
        metadata: {
          color: formData.get('color') as string,
          item_id: selectedTitle.metadata?.item_id
        }
      };
      ...
      const { error: shopError } = await supabase
        ...
        .eq('item_id', selectedTitle.metadata?.item_id);
  ...
  const handleDelete = async (title: DisplayTitle) => {
    ...
    try {
      ...
      await supabase
        .from('shop_items')
        .delete()
        .eq('item_id', title.metadata?.item_id);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // First, create an item in the items table
      ...
      const titleData = {
        ...
        metadata: {
          color: formData.get('color') as string,
          item_id: item.id // Store the reference to the item
        }
      };
      ...
      const { error: shopError } = await supabase
        .from('shop_items')
        .insert({
          item_id: item.id, // Use the item ID, not the title ID
          price: titleData.price,
          is_available: true,
          is_featured: formData.get('is_featured') === 'true',
          stock: null,
