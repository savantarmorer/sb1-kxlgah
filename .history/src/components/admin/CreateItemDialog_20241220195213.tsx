...
export const CreateItemDialog: React.FC<CreateItemDialogProps> = ({
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Only add non-empty metadata to avoid large objects
      if (Object.keys(formData.metadata).length > 0) {
        const metadataStr = JSON.stringify(formData.metadata);
        if (metadataStr.length <= 1000) { // Limit metadata size
          preparedData.metadata = formData.metadata;
        }
      }
