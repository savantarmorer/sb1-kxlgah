    ...
    try {
      ...
      const { error: shopError } = await supabase
        ...
        .update({
          ...
          data: {
            ...titleData,
            type: 'title' // Only include type in the shop_items data
          }
        })
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const titleData = {
        ...
        price: parseInt(formData.get('price') as string, 10),
        rarity: formData.get('rarity') as DisplayTitle['rarity'],
        type: 'title', // Important: specify this is a title type
        is_active: true,
        requirements: {
          level: parseInt(formData.get('required_level') as string, 10) || 0
        },
      ...
      const { error: shopError } = await supabase
        ...
        .insert({
          ...
          is_available: true,
          is_featured: formData.get('is_featured') === 'true',
          type: 'title', // Specify this is a title type
          data: titleData // Store the title data
        });
