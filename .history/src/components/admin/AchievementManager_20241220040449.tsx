...
export function AchievementManager() {
  ...
  const handleCreateAchievement = async (achievementData: {
    ...
    milestones: { progress: number; description: string; rewards: { type: string; value: number }[] }[];
  }) => {
    try {
      setIsLoading(true);
      const achievement = await AchievementService.create({
        ...
        trigger_conditions: achievementData.trigger_conditions
      });

      // Add rewards
      if (achievementData.rewards && achievementData.rewards.length > 0) {
        for (const reward of achievementData.rewards) {
          await AchievementService.addReward(achievement.id, reward);
        }
      }

      // Add milestones and their rewards
      if (achievementData.milestones && achievementData.milestones.length > 0) {
        for (const milestone of achievementData.milestones) {
          const createdMilestone = await AchievementService.addMilestone(achievement.id, {
          ...
          if (milestone.rewards && milestone.rewards.length > 0) {
            for (const reward of milestone.rewards) {
              await AchievementService.addMilestoneReward(createdMilestone.id, reward);
            }
          }
        }
      }
  ...
  const handleEditAchievement = async (achievementData: AchievementFormData) => {
  ...
  const handleEditClick = async (achievement: Achievement) => {
    try {
      ...
      setRewards(achievementRewards.map(r => ({
        type: r.type,
        value: r.value
      })));
