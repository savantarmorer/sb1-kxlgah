...
import { Code, Save, Eye, EyeOff, Grid as GridIcon, Settings, X, Keyboard, Plus, Link, Layers, Move, Play, Zap, Undo2, Redo2, Search, Command, Sun, Moon, RotateCw, AlignLeft, AlignCenter, AlignRight, AlignJustify, AlignHorizontalJustifyCenter, AlignVerticalJustifyCenter, Group, Ungroup, Lock, Unlock, ArrowUpToLine, ArrowDownToLine, ArrowUp, ArrowDown } from 'lucide-react';
...
export function EnhancedVisualEditor() {
  ...
  return (
    ...
    >
      ...
      <div 
        className={`fixed right-4 top-16 w-64 bg-white dark:bg-gray-800 p-4 rounded-lg shadow-lg pointer-events-auto transition-opacity duration-200 ${
          isActive && showPropertiesPanel && selectedComponent ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-4'
        }`}
      ...
      {/* Add resize handles */}
      {isActive && selectedComponent && renderResizeHandles()}

      {/* Add rotation handle */}
      {isActive && selectedComponent && renderRotationHandle()}

      {/* Add alignment toolbar */}
      {isActive && renderAlignmentToolbar()}

      {/* Add group controls */}
      {renderGroupControls()}

      {/* Add lock indicators */}
      {isActive && editableComponents.map(component => renderLockIndicator(component))}
    </div>
  );
}
