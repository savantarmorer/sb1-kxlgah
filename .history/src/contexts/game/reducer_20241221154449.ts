  ...
  switch (action.type) {
    ...
    case 'INITIALIZE_BATTLE':
      ...
      const battleMetadata: BattleMetadata = {
        is_bot: payload.opponent?.is_bot ?? true,
        difficulty: payload.opponent?.difficulty || 1,
        mode: (payload.metadata?.mode || 'practice') as BattleMetadata['mode']
      };
    ...
    case 'ANSWER_QUESTION':
      ...
      return {
        ...
        battle: {
          ...
          player_states: {
            ...state.battle.player_states,
            [state.user.id]: {
              ...state.battle.player_states[state.user.id],
              answered: true,
              answer: action.payload.answer,
              score: newScore.player,
              correct_count: state.battle.player_states[state.user.id].correct_count + (action.payload.is_correct ? 1 : 0)
            }
          }
        }
      };
    ...
    case 'END_BATTLE':
      ...
      return {
        ...
        battle: {
          ...
          in_progress: false,
          time_left: 0,
          current_question: state.battle.questions.length - 1
        }
      };
