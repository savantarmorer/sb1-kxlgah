...
export const handleBattleStateUpdate = (
  ...
  switch (action.type) {
    ...
    case 'SET_BATTLE_STATUS':
      ...
      if (action.payload === BattleStateEnum.ACTIVE) {
        if (!battleState.questions || battleState.questions.length === 0) {
          return {
            ...battleState,
            status: BattleStateEnum.ERROR,
            phase: BattleStateEnum.ERROR,
            error: {
              message: 'Cannot start battle: No questions available',
              timestamp: Date.now()
            }
      ...
      return {
        ...
        time_left: action.payload === BattleStateEnum.ACTIVE 
    ...
    case 'ANSWER_QUESTION': {
      ...
      return {
        ...
        score: newScore,
        status: isLastQuestion ? BattleStateEnum.COMPLETED : battleState.status,
    ...
    case 'END_BATTLE':
      ...
      if (player > opponent) finalStatus = BattleStateEnum.VICTORY;
      else if (opponent > player) finalStatus = BattleStateEnum.DEFEAT;
      else finalStatus = BattleStateEnum.DRAW;
