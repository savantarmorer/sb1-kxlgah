  ...
  handleItemTransaction: async (
    ...
    try {
      ...
      if (type === 'use' && existingItemIndex >= 0 && updatedInventory[existingItemIndex]?.quantity <= 0) {
        ...
        // Handle single item update/insert
        const { error: upsertError } = await supabase
          ...
          .upsert({
            ...
            quantity: type === 'purchase' 
              ? (existingItemIndex >= 0 ? updatedInventory[existingItemIndex].quantity : quantity)
              : (existingItemIndex >= 0 ? updatedInventory[existingItemIndex].quantity : 0),
            acquired_at: new Date().toISOString(),
            equipped: false
          });
