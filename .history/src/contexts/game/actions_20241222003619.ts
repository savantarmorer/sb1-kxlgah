...
export const gameActions = {
  ...
  handleItemTransaction: async (
    ...
    try {
      ...
      if (type === 'use' && existingItemIndex >= 0 && updatedInventory[existingItemIndex]?.quantity <= 0) {
      ...
      } else if (type === 'purchase' && existingItemIndex === -1) {
        // For new purchases, directly insert
        ...
        if (insertError) {
          console.error('[handleItemTransaction] Insert error:', insertError);
          throw insertError;
        }
        ...
        if (updateError) {
          console.error('[handleItemTransaction] Update error:', updateError);
          throw updateError;
        }
