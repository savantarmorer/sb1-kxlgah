...
export const GameProvider: React.FC<GameProviderProps> = ({ 
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = async () => {
      // Only refresh if:
      // 1. Document becomes visible
      // 2. User is logged in
      // 3. Not already refreshing
      // 4. App was hidden for more than 5 minutes
      if (
        document.visibilityState === 'visible' && 
        state.user?.id && 
        !isRefreshing
      ) {
        const lastActiveTime = parseInt(sessionStorage.getItem(`lastActiveTime_${state.user.id}`) || '0');
        const currentTime = Date.now();
        const timeDiff = currentTime - lastActiveTime;
        
        // Only refresh if the app was hidden for more than 5 minutes
        if (timeDiff > 5 * 60 * 1000) {
          console.debug('[GameContext] App was hidden for more than 5 minutes, refreshing game state');
          isRefreshing = true;
          
          try {
            const gameState = await retryOperation(async () => {
              return await BattleService.getCurrentGameState(state.user!.id);
            });
            ...
            console.error('[GameContext] Failed to refresh game state:', error);
          } finally {
            isRefreshing = false;
          }
        }
      ...
      // Always update last active time when visibility changes
      if (state.user?.id) {
