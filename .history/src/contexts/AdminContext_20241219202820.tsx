...
export function AdminProvider({ children }: { children: React.ReactNode }) {
  ...
  const debugActions = {
    ...
    modifyRewards: (rewards: Record<string, unknown>) => {
      ...
      dispatch({ 
        ...
        payload: { 
          ...
          xp: (rewards.xp as number) || state.user?.xp || 0,
          coins: (rewards.coins as number) || state.user?.coins || 0,
    ...
    simulateLogin: () => {
      ...
      dispatch({ 
        type: 'UPDATE_USER_PROGRESS', 
        payload: { streak: 0, level: state.user?.level || 1, xp: state.user?.xp || 0, coins: state.user?.coins || 0 }
      });
