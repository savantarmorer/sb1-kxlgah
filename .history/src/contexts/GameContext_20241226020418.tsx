    ...
    const handleVisibilityChange = async () => {
      if (document.visibilityState === 'visible' && state.user?.id && !isRefreshing) {
        ...
        try {
          ...
          if (gameState && gameState.user) {
            console.debug('[GameContext] Received game state:', gameState);
            ...
            if (gameState.battle_stats) {
              console.debug('[GameContext] Restoring battle stats:', gameState.battle_stats);
              updates.push({
                type: 'UPDATE_USER_PROFILE',
                payload: {
                  ...gameState.user,
            ...
            if (gameState.statistics) {
              console.debug('[GameContext] Restoring statistics:', gameState.statistics);
              updates.push({
                type: 'UPDATE_USER_PROFILE',
                payload: {
                  ...gameState.user,
            ...
            if (gameState.battle && gameState.battle.status !== 'idle') {
              console.debug('[GameContext] Restoring battle state:', gameState.battle);
              updates.push({
                type: 'UPDATE_BATTLE_PROGRESS',
                payload: {
                  ...gameState.battle,
            ...
            if (gameState.inventory) {
              console.debug('[GameContext] Updating inventory:', gameState.inventory);
              updates.push({
                type: 'UPDATE_INVENTORY',
                payload: { 
            ...
            console.debug('[GameContext] Applying state updates:', updates);
            ...
            console.debug('[GameContext] Saving state to localStorage:', stateToSave);
            localStorage.setItem('gameState', JSON.stringify(stateToSave));

            console.debug('[GameContext] State refresh completed successfully');
          } else {
            console.error('[GameContext] Received invalid game state:', gameState);
