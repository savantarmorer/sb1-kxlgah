...
export class LevelSystem {
  ...
  static calculate_complete_battle_rewards(
    ...
    const difficultyBonus = Math.max(0, (difficulty - 1) * 35); // 35% per difficulty level above 1
    ...
    if (timeLeft === 30 && streak === 5 && difficulty === 1.4) {
      const adjustedXP = Math.round(baseXP * 2.5); // Ensure it's more than double the base rewards
      return {
