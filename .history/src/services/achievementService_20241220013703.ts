  ...
  static async checkTriggers(
    ...
    for (const achievement of achievements || []) {
      ...
      const matchingTriggers = achievement.trigger_conditions?.filter(
        (t: any) => t.type === triggerType || triggerMap[t.type] !== undefined
      );
      ...
      const triggerProgresses = matchingTriggers.map(trigger => {
      ...
      const overallProgress = Math.min(...triggerProgresses.map(t => t.progress));
      // Achievement should unlock if all triggers are satisfied
      const shouldUnlock = triggerProgresses.every(t => t.shouldUnlock);
