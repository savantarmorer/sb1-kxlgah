...
export class AchievementService {
  ...
  static async checkTriggers(
    ...
    // Get all achievements with this trigger type, regardless of unlock status
    const { data: achievements, error } = await supabase
      .from('achievements')
      .select(`
        *,
      ...
      `)
      .contains('trigger_conditions', [{ type: triggerType }].map(JSON.stringify))
      .eq('user_achievements.user_id', userId)
      .order('order_num', { ascending: true });
    ...
    for (const achievement of achievements || []) {
      ...
      switch (triggerCondition.comparison) {
        case 'eq':
          progressPercentage = value === triggerCondition.value ? 100 : Math.min(100, (value / triggerCondition.value) * 100);
          break;
        case 'gt':
        case 'gte':
          progressPercentage = Math.min(100, (value / triggerCondition.value) * 100);
          break;
        case 'lt':
        case 'lte':
          progressPercentage = value <= triggerCondition.value ? 100 : Math.max(0, 100 - ((value - triggerCondition.value) / triggerCondition.value) * 100);
          break;
        default:
