...
export class LiveMatchmakingService {
  ...
  private static battleChannels: Map<string, RealtimeChannel> = new Map();
  ...
  private static async handleMatchFound(payload: { matchId: string; players: string[] }): Promise<void> {
    ...
    try {
      ...
      this.battleChannels.set(matchId, battleChannel);

      // Set up battle channel listeners
      await battleChannel
        .on('presence', { event: 'sync' }, () => {
          const state = battleChannel.presenceState();
          const readyPlayers = Object.values(state)
            .flat()
            .filter(p => (p as any).ready === true)
            .map(p => (p as any).player)
            .filter(Boolean);
            
          console.log('[Battle] Presence sync:', { 
            state, 
            readyPlayers,
            totalReady: readyPlayers.length 
          });
          
          // Only initialize battle when both players are ready
          if (readyPlayers.length === 2) {
            this.initializeBattle(matchId, readyPlayers);
          }
        })
        .on('broadcast', { event: 'player_ready' }, ({ payload: { playerId } }) => {
          console.log('[Battle] Player ready:', playerId);
          battleChannel.track({ 
            player: playerId,
            ready: true,
            joinedAt: Date.now()
      ...
      // Initially track players as not ready
      ...
      // Notify players that match is found and waiting for confirmation
      const [player1Details, player2Details] = await Promise.all([
        this.fetchPlayerDetails(players[0]),
        this.fetchPlayerDetails(players[1])
      ]);
      ...
      // Notify each player with their opponent's details
      this.notifyStateChange(players[0], {
        status: 'matched',
        matchId,
        opponent: player2Details
      });

      this.notifyStateChange(players[1], {
        status: 'matched',
        matchId,
  ...
  /**
   * Initialize battle with shared questions
   */
  private static async initializeBattle(matchId: string, players: string[]): Promise<void> {
    try {
      ...
      // Notify both players that battle is ready to start
      players.forEach(playerId => {
        this.notifyStateChange(playerId, {
          status: 'ready',
          matchId
      ...
      // Broadcast battle start after a delay to ensure modal shows
      setTimeout(() => {
        const battleChannel = this.battleChannels.get(matchId);
        if (battleChannel) {
          battleChannel.send({
            type: 'broadcast',
            event: 'battle_start',
      ...
      }, 5000); // 5 second delay for pre-battle modal

    } catch (error) {
      console.error('[Battle] Error initializing battle:', error);
      throw error;
    }
  }
   ...
   */
  static async confirmPlayerReady(matchId: string, playerId: string): Promise<void> {
    try {
      const battleChannel = this.battleChannels.get(matchId);
      if (!battleChannel) {
      ...
      console.error('[Battle] Error confirming player ready:', error);
      throw error;
    }
  }
