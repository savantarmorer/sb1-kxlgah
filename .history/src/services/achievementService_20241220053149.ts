...
export class AchievementService {
  ...
  static async getUserAchievements(userId: string): Promise<Achievement[]> {
    try {
      ...
      // Fetch achievements with user progress and rewards
      ...
      const processedAchievements = achievements.map(achievement => {
        ...
        return {
          ...
          rewards: achievement.achievement_rewards,
          milestones: achievement.achievement_milestones.map(milestone => ({
            ...milestone,
            reward: milestone.milestone_rewards?.[0]
          }))
        };
      });
