...
import { Achievement, AchievementProgress, TriggerCondition, AchievementRarity, AchievementReward, AchievementMilestone, MilestoneReward, AchievementMetadata } from '../types/achievements';
...
export class AchievementService {
  ...
  private static claimEventListeners: Set<() => void> = new Set();

  static subscribeToClaimEvents(listener: () => void): () => void {
    this.claimEventListeners.add(listener);
    return () => {
      this.claimEventListeners.delete(listener);
    };
  }

  private static notifyClaimEvent() {
    this.claimEventListeners.forEach(listener => listener());
  }
  ...
  static async getUserAchievements(userId: string): Promise<Achievement[]> {
    try {
      // Check cache first
      const cachedData = this.achievementsCache.get(userId);
      ...
      // Fetch rewards for each achievement
      const achievementsWithRewards = await Promise.all(
        achievements.map(async (achievement) => {
          ...
          return {
            ...
            ready_to_claim: userAchievement.ready_to_claim,
  ...
  static async create(achievement: Partial<Achievement>): Promise<Achievement> {
    ...
    const { metadata: metadataInput = { icon: '🏆' }, progress, unlocked, unlocked_at, ...rest } = achievement;
    ...
    try {
      ...
      return {
        ...newAchievement,
        rarity: newAchievement.rarity as AchievementRarity,
        metadata: {
          ...newAchievement.metadata,
          icon: newAchievement.metadata?.icon || '🏆'
        }
      };
    } catch (error) {
  ...
  static async update(id: string, achievement: Partial<Achievement>): Promise<Achievement> {
    try {
      ...
      // Prepare the achievement update data
      ...
      if (dependents) updateData.dependents = dependents;
      if (metadata) {
        updateData.metadata = {
          ...(metadata || {}),
          icon: metadata.icon || '🏆'
      ...
      if (claimed !== undefined || ready_to_claim !== undefined || progress !== undefined || unlocked_at !== undefined) {
        ...
        if (claimed !== undefined) userAchievementData.claimed = claimed;
        if (ready_to_claim !== undefined) userAchievementData.ready_to_claim = ready_to_claim;
        if (progress !== undefined) userAchievementData.progress = progress;
        if (unlocked_at !== undefined) userAchievementData.unlocked_at = unlocked_at;

        const { error: userAchievementError } = await supabase
          .from('user_achievements')
          .update(userAchievementData)
      ...
      // Return processed achievement
      return {
        ...updatedAchievement,
        rarity: updatedAchievement.rarity as AchievementRarity,
        metadata: {
          ...updatedAchievement.metadata,
          icon: updatedAchievement.metadata?.icon || '🏆'
        }
      };
    } catch (error) {
  ...
  static async claimAchievement(userId: string, achievementId: string): Promise<boolean> {
    try {
      ...
      // Notify listeners of the claim event
      this.notifyClaimEvent();
  ...
  static async getAchievementRewards(achievementId: string): Promise<AchievementReward[]> {
    try {
      ...
      console.error('Error in getAchievementRewards:', error);
      throw error;
    }
  }
} 