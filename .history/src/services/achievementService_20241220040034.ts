...
export class AchievementService {
  ...
  static async addMilestone(achievementId: string, milestone: { progress: number; description: string }): Promise<AchievementMilestone> {
    ...
    if (error) throw error;
    if (!data) throw new Error('Failed to create milestone');
    return data;
  }
  ...
  static async addMilestoneReward(milestoneId: number, reward: { type: string; value: number }): Promise<MilestoneReward> {
    const { data, error } = await supabase
    ...
    if (error) throw error;
    if (!data) throw new Error('Failed to create milestone reward');
    return data;
  }
} 