...
export class AchievementService {
  ...
  static async claimAchievement(userId: string, achievementId: string): Promise<boolean> {
    ...
    const { data: achievementData } = await supabase
      ...
      .match({ 'user_achievements.user_id': userId, id: achievementId })
      .single();

    if (!achievementData?.user_achievements?.[0]?.ready_to_claim) {
    ...
    achievementData.rewards?.forEach((reward: Reward) => {
      switch (reward.type) {
        case 'coins':
        ...
        case 'xp':
          ...
          break;
        case 'title':
          titlesToAdd.push(String(reward.value));
          break;
        case 'avatar':
          avatarsToAdd.push(String(reward.value));
          break;
      }
    });

    // Process milestone rewards for reached milestones
    achievementData.milestones?.forEach((milestone: Milestone) => {
      if (achievementData.user_achievements[0].progress >= milestone.progress) {
        switch (milestone.reward.type) {
          ...
          case 'xp':
            ...
            break;
          case 'title':
            titlesToAdd.push(String(milestone.reward.value));
            break;
          case 'avatar':
            avatarsToAdd.push(String(milestone.reward.value));
            break;
        }
      }
    });
    ...
    const { error: profileError } = await supabase
      ...
      .update({
        ...
        // Add titles and avatars to user's collection if needed
    ...
    if (achievementError) {
      console.error('Error claiming achievement:', achievementError);
      return false;
    }
    ...
    const { error: notificationError } = await supabase
      ...
      .insert({
        ...
        message: `You received: ${[
          ...
          ...titlesToAdd.map(title => `Title: ${title}`),
          ...avatarsToAdd.map(avatar => `Avatar: ${avatar}`)
        ].filter(Boolean).join(', ')}`,
        data: {
          achievement_id: achievementId,
          rewards: {
    ...
    if (achievementData.dependents?.length > 0) {
  ...
  static async addReward(achievementId: string, reward: { type: string; value: number }) {
    const { data, error } = await supabase
  ...
  static async getRewards(achievementId: string) {
    const { data, error } = await supabase
      .from('achievement_rewards')
      .select('*')
  ...
  static async clearRewards(achievementId: string) {
    const { error } = await supabase
      .from('achievement_rewards')
      .delete()
      .eq('achievement_id', achievementId);
  ...
  static async addMilestone(achievementId: string, milestone: { progress: number; description: string }) {
    const { data, error } = await supabase
  ...
  static async getMilestones(achievementId: string) {
    ...
    return milestones.map(milestone => ({
      ...milestone,
      reward: milestone.reward[0] // Take the first reward since each milestone has one reward
    }));
  }

  static async clearMilestones(achievementId: string) {
    const { error } = await supabase
      .from('achievement_milestones')
      .delete()
      .eq('achievement_id', achievementId);
  ...
  static async addMilestoneReward(milestoneId: number, reward: { type: string; value: number }) {
    const { data, error } = await supabase
