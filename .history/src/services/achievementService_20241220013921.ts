...
export class AchievementService {
  ...
  static async checkTriggers(
    ...
    for (const achievement of achievements || []) {
      ...
      if (shouldUnlock && !achievement.user_achievements?.[0]?.unlocked_at) {
        batch.push(this.unlockAchievement(userId, achievement.id));
        ...
        if (achievement.dependents?.length > 0) {
          batch.push(this.checkDependentAchievements(userId, achievement.dependents));
        }
      } else {
        updatedAchievements.push({
          ...achievement,
  ...
  /**
   * Claims rewards for an unlocked achievement
   * @param userId - The ID of the user
   * @param achievementId - The ID of the achievement
   */
