...
export class BattleService {
  ...
  public static async getCurrentGameState(user_id: string): Promise<GameState> {
    ...
    const cachedState = sessionStorage.getItem(`gameState_${user_id}`);
    if (cachedState) {
      try {
        const parsedState = JSON.parse(cachedState);
        // Validate that the cached state has all required data
        if (parsedState && 
            parsedState.user && 
            parsedState.user.id === user_id &&
            parsedState.battle_stats &&
            parsedState.statistics &&
            parsedState.battle) {
          // Verify if the cached state is still fresh (less than 5 minutes old)
          const cacheTime = sessionStorage.getItem(`gameState_${user_id}_time`);
          if (cacheTime && Date.now() - Number(cacheTime) < 5 * 60 * 1000) {
            console.debug('[BattleService] Using cached game state', {
              user: !!parsedState.user,
              battle_stats: !!parsedState.battle_stats,
            ...
            console.debug('[BattleService] Cached state expired, fetching fresh state');
          }
        } else {
          console.debug('[BattleService] Cached state incomplete, fetching fresh state', {
            hasUser: !!parsedState?.user,
            hasStats: !!parsedState?.battle_stats,
            hasStatistics: !!parsedState?.statistics,
            hasBattle: !!parsedState?.battle
          });
        }
      } catch (error) {
        console.warn('[BattleService] Failed to parse cached state:', error);
      }
    }
    ...
    try {
      // Get user profile first as it's most critical
      ...
      if (user_error) {
        console.error('[BattleService] Failed to get user profile:', user_error);
        throw user_error;
      }
      ...
      // Get battle stats - don't fail if this errors
      try {
        const { data: battle_stats } = await supabase
          .from('battle_stats')
          .select('*')
          .eq('user_id', user_id)
          .single();
        ...
        if (battle_stats) {
          gameState.battle_stats = battle_stats;
          gameState.statistics = {
            ...gameState.statistics,
            total_xp: user.xp || 0,
            total_coins: user.coins || 0,
            battles_won: battle_stats.wins,
            battles_lost: battle_stats.losses,
            current_streak: battle_stats.win_streak,
            highest_streak: battle_stats.highest_streak
          };
        }
      } catch (stats_error) {
        console.warn('[BattleService] Failed to get battle stats:', stats_error);
      }

      // Get inventory - don't fail if this errors
      try {
        const { data: inventory_items } = await supabase
          .from('user_inventory')
          .select(`
            id,
            user_id,
          ...
          `)
          .eq('user_id', user_id) as { data: InventoryItemResponse[] | null };
        ...
        if (inventory_items) {
          gameState.inventory = {
            items: inventory_items.map(inv => ({
              ...
              type: inv.items?.type as ItemType || ItemType.CONSUMABLE,
              rarity: inv.items?.rarity as ItemRarity || ItemRarity.COMMON,
      ...
      } catch (inventory_error) {
        console.warn('[BattleService] Failed to get inventory:', inventory_error);
      }

      // Cache the successful state with timestamp
      try {
        sessionStorage.setItem(`gameState_${user_id}`, JSON.stringify(gameState));
        sessionStorage.setItem(`gameState_${user_id}_time`, Date.now().toString());
        console.debug('[BattleService] Cached game state with timestamp');
      } catch (cache_error) {
        console.warn('[BattleService] Failed to cache game state:', cache_error);
      }

      return gameState;

    } catch (error) {
      console.error('[BattleService] Failed to get game state:', error);
      // Try to get cached state as fallback
      const cachedState = sessionStorage.getItem(`gameState_${user_id}`);
      if (cachedState) {
        try {
          const parsedState = JSON.parse(cachedState);
          if (parsedState && parsedState.user && parsedState.user.id === user_id) {
            console.debug('[BattleService] Using cached state as fallback');
            return parsedState;
          }
        } catch (parse_error) {
          console.error('[BattleService] Failed to parse cached state:', parse_error);
        }
      }
      throw error;
    }
   ...
   */
  public static handleBattleStateUpdate = async (
    ...
    switch (action.type) {
      ...
      case 'SET_BATTLE_STATUS':
        dispatch({
          type: 'SET_BATTLE_STATUS',
          payload: action.payload as BattleStateEnum
        });
        break;
    }
  };
...
export async function handle_battle_results(
  ...
  const results: BattleResults = {
    ...
    user_id: user_id,
    score: {
      player: battle_state.score.player,
      opponent: battle_state.score.opponent
    },
    rewards: rewards,
    stats: {
      ...
      average_time: 0,
      correct_answers: battle_state.score.player || 0
    }
  };
