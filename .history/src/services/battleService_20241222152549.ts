...
export class BattleService {
  ...
  public static async getCurrentGameState(user_id: string): Promise<GameState> {
    ...
    console.log('Transformed inventory:', inventory);
  ...
  /**
   * Applies the effect of an inventory item during a battle
   * @param userId - ID of the user using the item
   * @param itemId - ID of the item being used
   */
  public static async applyItemEffect(userId: string, itemId: string): Promise<void> {
    try {
      ...
      switch (item.type) {
        case 'alternative_remover_potion':
          // Logic to remove alternative answers
          // This might involve updating the battle state in the database
          await supabase
            .from('battle_matches')
            .update({
              metadata: {
                ...currentMetadata,
                // Assuming you store eliminated options in metadata
                eliminated_options: ['B', 'C'] // Example: remove options B and C
              }
            })
            .eq('user_id', userId);
          break;
      ...
      console.error('[BattleService] Failed to apply item effect:', error);
      throw error;
    }
  }
}
